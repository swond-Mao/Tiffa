...
- **git 里实际保留的关键资产**：`tiffa-desktop.exe`(启动器，IN-GIT)、`electron/package.json`(IN-GIT)、`data/agent/models.yml.example`(IN-GIT，install 据此生成 models.yml)、skill 代码。**embedding 模型已进 git（普通文件，非 LFS）**：真正运行用的是 `home/.omp/cache/fastembed/fast-bge-small-zh-v1.5`（93MB，config 确认 `BAAI/bge-small-zh-v1.5` 512d）。已把该模型原样提交到 `embedding-assets/fast-bge-small-zh-v1.5/`（`model_optimized.onnx` 真实 90MB blob，绕开 `home/` 忽略链），install 拷到 `home/.omp/cache/fastembed/`。**为何不用 LFS**：gitee 免费版直接报「LFS only supported in paid/trial enterprise」不支持 LFS；gitcode LFS 传 90MB 大文件也被远端强制断连不稳。故放弃 LFS，改普通文件提交（gitee/gitcode 均原生存，clone 即带）。`.gitattributes` 里 force 保留的 `data/agent/mnemopi/` 4 文件是**死配置**（该目录本机不存在）；`local_cache/fast-bge-small-zh-v1.5`（1.3MB，仅 tokenizer 无权重）也是误导项。注意 `fastembed-runtime`(870MB onnxruntime) 仍不进 git，靠首次启用时内核 bun 从 npmmirror 拉或源机拷贝。
- **分发方案已定（2026-08-05 落地）**：目标「git clone + install 从国内源拉依赖 + 就地安装」。最终设计（全部国内可达，embedding 权重作普通文件提交）：① **embedding 权重** `BAAI/bge-small-zh-v1.5`（94MB `model_optimized.onnx`）→ 直接提交到 `embedding-assets/fast-bge-small-zh-v1.5/`（真实 blob，非 LFS），绕开 `home/` 忽略链，install 拷到 `home/.omp/cache/fastembed/`；② **Node** v22.17.1 → install 从 `registry.npmmirror.com/-/binary/node` 下 zip 解压到 `node/`；③ **Python** 3.13.12 → install 下 embeddable 基础包（npmmirror）+ `ensurepip`/`get-pip.py`(清华) + `pip install -r requirements-python.txt`(清华)；**无需 LFS/单独包**（关键发现：`python/tesseract` 本就是空目录、代码不调用 tesseract，环境 100% pip 可重建）；④ **fastembed-runtime**(870MB onnxruntime) → 首次启用记忆时由内核 bun 从 npmmirror 自动拉（根 `.npmrc` 已写国内源）；若失败 install 提示从源机拷贝 `home/.omp/cache/fastembed-runtime/`；⑤ **bun+内核** 维持原 install 经 npmmirror 装。安装位置：就地（clone/copy 到哪装到哪）。已改 `install.ps1`（语法校验通过）、新增 `embedding-assets/`、`requirements-python.txt`、根 `.npmrc`（install 运行时写）。注意：**gitee 不支持 LFS**，embedding 必须走普通文件提交；`git push` 直接传 90MB pack 较慢（gitee 实测约 20 分钟）但可用。
