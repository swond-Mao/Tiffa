...
1. **版式设计语言（L-*）**：每个模板标注版式语言（L-chart/L-rail/L-quote/L-fullbleed/L-bento/L-ghost/L-table/L-asym/L-定位）——选模板时**知道它怎么排布**，吸收其布局意图后用 DSL 复刻（不填空）
2. **主题锁定（风格隔离）**：全篇一个 themePack
3. **内容类型**：19 类
4. **布局族**：L3（纯文字/多卡片/并列/流程/…）
5. **容量**：卡数/图数/文本量
