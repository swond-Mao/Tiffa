...
> **按受众定高级排版比例**（见 3.5 受众视觉密度决策）：读 `references/advanced-typeset.md`（无卡片手法 T1-T10：文字骑图/出血字/细线分隔/负空间/编号/金线骑字/全页大图/竖排/**渐进缩放/文字倾斜 rotate** + 浮夸元素）。**稳重汇报**以卡片为主 + 少量高级页（封面/金句/结尾）；**商业推广**以高级排版为主；**战略运营**混合。内容需要"独立强调块"才用卡片；高级排版页用文字骑图/出血字/细线分隔/负空间/倾斜/渐进，**禁止每个要点都塞卡片框**。奢华/浮夸风格的高级排版页必加鎏金粒子 + 光晕 + 渐变条 + 微倾张力。
