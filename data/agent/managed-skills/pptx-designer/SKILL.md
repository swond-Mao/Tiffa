...
3.5 **受众 → 视觉密度决策**（关键：高级排版页 vs 卡片页的比例）：
   - **稳重汇报**（医院/政府/学术/老头子领导/学术答辩）：**卡片为主**（规整清晰不浮夸）+ **少量高级页**（封面/章节/金句/结尾 1-3 页）做记忆点。高级排版比例 ≤20%
   - **商业推广**（品牌推介/商业路演/产品发布）：**高级排版为主**（文字骑图/出血字/全页大图/粒子）+ 卡片辅助。高级排版比例 ≥60%
   - **战略运营**（战略汇报/运营方案/投资人）：**混合**（卡片承载数据/结构 + 高级排版做视觉冲击页）。高级排版比例 40-60%
   - 判断依据：受众年龄/场合正式度/内容抽象度——越正式越稳重（多卡片），越商业越浮夸（多高级排版）
...
> **按受众定高级排版比例**（见 3.5 受众视觉密度决策）：读 `references/advanced-typeset.md`（无卡片手法 T1-T11：文字骑图/出血字/细线分隔/负空间/编号/金线骑字/全页大图/竖排/渐进缩放/文字倾斜 rotate/斜拉装饰 diagonal + 浮夸元素）。**稳重汇报**以卡片为主 + 少量高级页（封面/金句/结尾）；**商业推广**以高级排版为主；**战略运营**混合。内容需要"独立强调块"才用卡片；高级排版页用文字骑图/出血字/细线分隔/负空间/倾斜/渐进/斜拉，**禁止每个要点都塞卡片框**。奢华/浮夸风格的高级排版页必加鎏金粒子 + 光晕 + 渐变条 + 微倾张力 + 斜拉装饰。

读 `references/methodology.md` §四 决策矩阵 + `references/design-principle.md`，每页声明：
- **本页结论**（一句话，说不清这页不过）
- **文字内容与分布**：标题/正文/数字，哪些字放哪块区域
- **卡片格式**：几块卡片/色块/列表/表格，内容怎么装
- **图片张数与图位（设计前置，关键）**：0-4 张，**先定每页图位坐标 (x, y, w, h) + 比例，再按图位比例生成对应比例图**——避免"图生成完发现尺寸/比例不对"。图位比例决定生成比例（竖卡 3:4 → 1080×1920；方卡 1:1 → 1024×1024；全页大图 16:9 → 1920×1080）
- **素材清单（生图前置）**：汇总全 deck 所有图位 → 列出素材清单（每张图的用途/比例/位置）→ 批量生成（独立 seed，避免雷同）→ 预处理（fit 函数按图位比例裁切缩放，绝不拉伸）→ 看每张质量
- **配图清单**：需生图的先走 image-gen 出图到 resources/images/（读 `comfyui-image-gen/image-gen-playbook.md` 批量素材策略）
