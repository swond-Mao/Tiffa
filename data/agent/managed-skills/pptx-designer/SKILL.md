...
> **默认高级排版，卡片是例外**：读 `references/advanced-typeset.md`（无卡片手法 T1-T8：文字骑图/出血大字/细线分隔/负空间留白/编号文字/金线骑字/全页大图/竖排 + 浮夸元素：粒子/光晕/渐变条）。内容需要"独立强调块"才用卡片；能靠文字/图/线/留白构成的，一律用无卡片手法。**禁止每个要点都塞卡片框**（卡片依赖 = 平庸）。奢华/浮夸风格必加鎏金粒子 + 光晕 + 渐变条。
