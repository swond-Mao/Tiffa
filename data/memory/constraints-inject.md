...
- **踩坑记档**：仅确定踩坑（根因明确+修复验证）**全量写入全局库卡**（`mcp__mnemopi_remember`，格式 `[类型]踩坑 [范围]通用|<项目名> <结论> 根因/修复/关键文件/教训`，控制在 800 字内）；提及以前实现先 recall 语义召回，命中即读卡全文。勿写 PROJECT.md、勿维护项目 docs/踩坑记录.md（旧文件仅作历史存档）
...
任务完成后必须**直接输出结论**结束本轮，不要再用工具"试探"自己是否完成：

- **判断完成的依据**是客观产物（文件已落盘、命令已验证、todo 无 open 项），不是"感觉差不多了"
- **禁止占位命令**：`echo xxx`、`true`、`pwd`、`sleep` 这类不改变任何状态的命令假装在干活 —— 连续出现会被框架拦截，bash 还会被临时移出工具列表
- **禁止意图与动作背离**：不要一边写"改用 write 工具"一边继续调 bash。要调哪个工具就直接调，不写占位串
- **确实还有活**：先用 `todo` 把它记为 `in_progress` 再动手；若在等外部输入（用户确认、外部系统），标 `blocked`
- 收尾输出格式：**做了什么 → 改了哪些文件 → 验证结果 → 遗留风险/下一步**
