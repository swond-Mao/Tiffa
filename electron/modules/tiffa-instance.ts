...
export class TiffaInstance {
  ...
  get busyReason(): string {    const idle = Math.round((Date.now() - this.lastActiveTime) / 1000);
    ...
    if (this._pendingAskIds.size > 0) parts.push(`${this._pendingAskIds.size} 个确认框在等你答复`);
  ...
  private _handleEvent(event: TiffaEvent): void {
    ...
    if (event.type === 'prompt_result') {
      if (event.agentInvoked) {
        ...
        mainLog(`[${this._shortCwd()}#${this.sessionId}] prompt_result agentInvoked=false（已排队，未发起模型请求）`);
      }
  ...
  announceNewSessionReady(): void {
    if (!this.sessionFilePath) return;
    if (!_mainWindow || _mainWindow.isDestroyed()) return;
    const realId = extractSessionIdFromPath(this.sessionFilePath) || this.sessionId;
    _mainWindow.webContents.send('tiffa:event', {
      type: 'session_switch',
      reason: 'new',
