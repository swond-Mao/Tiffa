...
function fetchHtml(url: string, baseUrl: string): Promise<string> {
	return new Promise((resolve, reject) => {
		const doGet = (target: string, follow: boolean) => {
			https
				.get(
					target,
					{
					...
					(res) => {
						if (
							...
							const next = new URL(res.headers.location, baseUrl);
...
function parseBing(html: string, max: number): SearchResult[] {
	...
	for (const b of blocks) {
		...
		if (url && title) results.push({ title, url, content, engine: "bing" });
		if (results.length >= max) break;
	}
	return results;
}

function parseSo360(html: string, max: number): SearchResult[] {
	...
	for (const b of blocks) {
		...
		if (url && title) results.push({ title, url, content, engine: "so360" });
		if (results.length >= max) break;
	}
	return results;
}
...
export function startWebSearchProxy(): Promise<number> {
	return new Promise((resolve, reject) => {
