...
const CONTEXT_OVERFLOW_RE = /prompt is too long|input is too long|exceeds?( the)?( model'?s?)?( maximum)? context|maximum context length|context (window|length|size).{0,20}(exceeded|overflow|too small)|too many tokens|token limit exceeded|exceeds the limit of \d+ tokens|n_ctx|requested tokens?.{0,20}exceed/i;
...
class TiffaInstance {
    ...
    kill(sync = false) {
        ...
        for (const t of this._askTimeouts.values())
            clearTimeout(t);
    ...
    cancelPendingAsks(reason) {
        ...
        (0, session_utils_1.mainLog)(`[${this._shortCwd()}#${this.sessionId}] 已取消挂起审批 ${ids.length} 个（${reason}）`);
        return ids.length;
    }
    /**
    ...
    forceReset(reason) {
        ...
        (0, session_utils_1.mainLog)(`[${this._shortCwd()}#${this.sessionId}] 强制复位运行态 wasRunning=${wasRunning}（${reason}）`);
        if (_mainWindow && !_mainWindow.isDestroyed()) {
            _mainWindow.webContents.send('tiffa:event', {
                type: 'agent_end',
                messages: [],
     ...
     */
    _maybeRecoverContextOverflow(event) {
        if (event._wasPrewarming)
            return;
        ...
        if (!errMsg || !CONTEXT_OVERFLOW_RE.test(errMsg))
            return;
        if (this._overflowRecovering)
            return;
        const now = Date.now();
        if (now - this._overflowRecoverAt < OVERFLOW_RECOVER_COOLDOWN_MS) {
            (0, session_utils_1.mainLog)(`[${this._shortCwd()}#${this.sessionId}] 上下文超限但处于恢复冷却期，跳过自动压缩`);
            return;
        }
        const prompt = this.lastPromptMessage;
        this._overflowRecoverAt = now;
        this._overflowRecovering = true;
        (0, session_utils_1.mainLog)(`[${this._shortCwd()}#${this.sessionId}] 上下文超限 -> 自动压缩+重试（err=${errMsg.slice(0, 160)}）`);
        this._sendNotice('warning', '上下文超出模型窗口，正在自动压缩会话并重试…');
        void (async () => {
            const flagPath = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'compact-force-next.json');
            try {
                // 强制扩展走 ③ 旁路结构化摘要（跳过 ①②snapcompact），与 main.ts 帧预算兜底同一机制
                fs_1.default.mkdirSync(path_1.default.dirname(flagPath), { recursive: true });
                fs_1.default.writeFileSync(flagPath, JSON.stringify({ ts: Date.now(), sessionId: this.sessionId }), 'utf8');
                try {
                ...
                (0, session_utils_1.mainLog)(`[${this._shortCwd()}#${this.sessionId}] 超限自动压缩完成`);
                if (!prompt) {
                    this._sendNotice('info', '已自动压缩会话，请重新发送你的消息');
                    return;
                }
                await this.sendCommand({ type: 'prompt', message: prompt });
                (0, session_utils_1.mainLog)(`[${this._shortCwd()}#${this.sessionId}] 超限恢复：已重发原消息`);
                this._sendNotice('info', '会话已压缩，原消息已自动重新发送');
            }
            catch (e) {
                const msg = e instanceof Error ? e.message : String(e);
                (0, session_utils_1.mainLog)(`[${this._shortCwd()}#${this.sessionId}] 超限自动恢复失败：${msg}`);
    ...
    armAbortWatchdog(onGiveUp) {
        ...
        const timer = setInterval(() => {
            ...
            if (this._lastRealAgentEndAt >= startedAt) {
                this._clearAbortWatchdog();
                (0, session_utils_1.mainLog)(`[${this._shortCwd()}#${this.sessionId}] abort 已生效（${Math.round((Date.now() - startedAt) / 1000)}s）`);
                return;
            }
            const elapsed = Date.now() - startedAt;
            if (elapsed >= ABORT_KILL_MS) {
                this._clearAbortWatchdog();
                (0, session_utils_1.mainLog)(`[${this._shortCwd()}#${this.sessionId}] abort 在 ${Math.round(ABORT_KILL_MS / 1000)}s 内未生效，终止实例以解除卡死`);
                try {
                    onGiveUp(this);
                }
                catch (e) {
     ...
     */
    _armAskTimeout(id) {
        if (!this.isUnattended || UNATTENDED_ASK_TIMEOUT_MS <= 0)
            return;
        this._clearAskTimeout(id);
        const timer = setTimeout(() => {
            this._askTimeouts.delete(id);
            if (!this._pendingAskIds.has(id))
                return;
            (0, session_utils_1.mainLog)(`[${this._shortCwd()}#${this.sessionId}] 无人值守实例审批挂起超时（${Math.round(UNATTENDED_ASK_TIMEOUT_MS / 60000)} 分钟），自动按「拒绝」应答 id=${id}`);
        ...
        if (event.type === 'ready') {
            ...
            if (Array.isArray(event.supportedProtocolVersions) && event.supportedProtocolVersions.includes(2)) {
                this.sendCommand({ type: 'negotiate_protocol', protocolVersion: 2 })
                    ...
                    .catch((e) => { console.warn(`[TiffaInstance:${this._shortCwd()}] RPC v2 协商失败，保持 v1: ${e.message}`); });
            }
            this.agentRunning = false;
            this.crashCount = 0;
