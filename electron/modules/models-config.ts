...
export const SUPPORTED_PROVIDER_APIS: readonly string[] = [
...
export function validateModelsConfig(data: unknown): string[] {
  ...
  for (const [pid, p] of Object.entries(providers as Record<string, unknown>)) {
    ...
    if (prov.api !== undefined && prov.api !== '' && !SUPPORTED_PROVIDER_APIS.includes(String(prov.api))) {
      errors.push(`供应商 ${pid}: API 类型 "${prov.api}" 不受支持（可选：${SUPPORTED_PROVIDER_APIS.join(' / ')}；Ollama/llama.cpp 请选 OpenAI 兼容并把地址改为 .../v1）`);
    }
 ...
 */
export function extractUnsupportedApiProviders(data: unknown): { removed: Record<string, _SanitizeAny> } {
  const removed: Record<string, _SanitizeAny> = {};
  const providers = (data as _SanitizeAny | null)?.providers;
  ...
  for (const [pid, p] of Object.entries(rec)) {
    ...
    const api = (p as _SanitizeAny).api;
    if (api !== undefined && api !== '' && !SUPPORTED_PROVIDER_APIS.includes(String(api))) {
      removed[pid] = p as _SanitizeAny;
      delete rec[pid];
    }
  }
  return { removed };
