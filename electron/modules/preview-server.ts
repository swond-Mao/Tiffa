...
export interface PreviewRegistration {
  ...
  /** 已登记的绝对路径 */
  file: string;
  /** 变更计数；每次文件改动 +1，面板拼进 ?v= 做缓存击穿 */
  rev: number;
}

/** registerPreview 结果：失败时带中文原因，直接透给 agent/用户看 */
export type PreviewRegisterResult = { ok: true } & PreviewRegistration | { ok: false; error: string };

interface PreviewEntry extends PreviewRegistration {
  /** 允许读取的根目录（已 resolve、无尾分隔符） */
  dir: string;
...
export function setPreviewMainWindowGetter(fn: () => BrowserWindow | null): void {
  mainWindowGetter = fn;
}
...
function serveFile(entry: PreviewEntry, relPath: string, res: http.ServerResponse): void {
  let decoded: string;
  try {
  ...
  const target = path.resolve(entry.dir, "." + path.sep + decoded.replace(/^[/\\]+/, ""));
...
function handleRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
  ...
  serveFile(entry, m[2] && m[2].length ? m[2] : path.basename(entry.file), res);
}

function notifyChanged(entry: PreviewEntry): void {
  entry.rev++;
...
function attachWatcher(entry: PreviewEntry): void {
  ...
  try {
    entry.watcher = fs.watch(entry.dir, { persistent: false }, (_event, filename) => {
      if (filename && path.basename(String(filename)) !== path.basename(entry.file)) return;
 ...
 */
export function registerPreview(absFile: string): PreviewRegisterResult {
  const file = path.resolve(absFile);
  if (!fs.existsSync(file) || !fs.statSync(file).isFile()) {
    return { ok: false, error: `文件不存在：${file}` };
  }
  if (!isInsideRoot(PORTABLE_ROOT, file)) {
  ...
  entry.url = `http://127.0.0.1:${actualPort}/preview/${entry.id}/${encodeURIComponent(path.basename(file))}`;
...
function snapshot(e: PreviewEntry): PreviewRegistration {
...
export function stopPreviewServer(): void {
  for (const id of [...entries.keys()]) unregisterPreview(id);
  if (!server) return;
  try {
    server.close();
  } catch {
