...
function setupIpc() {
    ...
    electron_1.ipcMain.handle('memory:recall', async (event, query) => {
        ...
        try {
            const pythonExe = path_1.default.join(constants_1.PORTABLE_ROOT, 'python', 'python.exe');
            const banksDir = path_1.default.join(constants_1.AGENT_DIR, 'memories', 'mnemopi', 'banks');
            const script = `
