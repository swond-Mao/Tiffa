...
if (argRootIdx >= 0 && process.argv[argRootIdx + 1]) {
    global.PORTABLE_ROOT = path_1.default.resolve(process.argv[argRootIdx + 1]);
}
else if (process.env.PORTABLE_ROOT) {
    global.PORTABLE_ROOT = path_1.default.resolve(process.env.PORTABLE_ROOT);
}
else {
    global.PORTABLE_ROOT = path_1.default.resolve(__dirname, '..');
}
// 自包含便携
// ── 自包含便携：把 Electron userData 锁到便携目录，localStorage/openTabs 等随 U 盘走 ──
// 必须在 app.ready 之前调用（此处模块顶层、BrowserWindow 创建前，时机正确）。
try {
    const portableUserData = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'electron-userdata');
    fs_1.default.mkdirSync(portableUserData, { recursive: true });
    electron_1.app.setPath('userData', portableUserData);
...
try {
    const CONFIG_YML = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'config.yml');
    const CONFIG_EXAMPLE = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'config.yml.example');
    if (!fs_1.default.existsSync(CONFIG_YML) && fs_1.default.existsSync(CONFIG_EXAMPLE)) {
        fs_1.default.copyFileSync(CONFIG_EXAMPLE, CONFIG_YML);
...
try {
    const MCP_JSON_HEAL = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'mcp.json');
    if (fs_1.default.existsSync(MCP_JSON_HEAL)) {
        let rawHeal = fs_1.default.readFileSync(MCP_JSON_HEAL, 'utf8');
        const rootSlash = constants_1.PORTABLE_ROOT.replace(/\\/g, '/');
        const fixed = rawHeal.replace(/[A-Za-z]:[\/\\]Tiffa[\/\\]/g, rootSlash + '/');
        if (fixed !== rawHeal) {
            fs_1.default.writeFileSync(MCP_JSON_HEAL, fixed, 'utf8');
...
try {
    ...
    const MODELS_EXAMPLE = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'models.yml.example');
    if (!fs_1.default.existsSync(MODELS_YML) && fs_1.default.existsSync(MODELS_EXAMPLE)) {
        fs_1.default.copyFileSync(MODELS_EXAMPLE, MODELS_YML);
...
function _closeInstancesForSessionFile(sessionPath) {
    ...
    const targetSessionId = (0, session_utils_1.extractSessionIdFromPath)(resolved);
    const keysToClose = [];
    for (const [key, inst] of tiffaManager.instances) {
        const sf = inst.sessionFilePath;
        const matchByPath = sf && path_1.default.resolve(sf).toLowerCase() === norm;
        const matchById = targetSessionId && inst.sessionId === targetSessionId;
        if (matchByPath || matchById)
...
process.on('uncaughtException', (err) => {
    try {
        ...
        (0, session_utils_1.mainLog)(`[main] uncaughtException: ${(err && err.stack) || err}`);
    }
    catch { }
});
process.on('unhandledRejection', (reason) => {
    try {
        ...
        (0, session_utils_1.mainLog)(`[main] unhandledRejection: ${(reason && reason.stack) || reason}`);
    }
    catch { }
});
...
function assertProjectCwd(cwd) {
    ...
    const rootNorm = path_1.default.resolve(constants_1.PORTABLE_ROOT).toLowerCase();
    const wsNorm = path_1.default.resolve(path_1.default.join(constants_1.PORTABLE_ROOT, 'workspace')).toLowerCase();
    const cwdNorm = normalized.toLowerCase();
    if (cwdNorm.startsWith(rootNorm + path_1.default.sep) && !cwdNorm.startsWith(wsNorm + path_1.default.sep)) {
...
function setupIpc() {
    ...
    // Tiffa commands
    electron_1.ipcMain.handle('tiffa:send', async (event, message, images, sessionId) => {
        ...
        if (omfgMatch) {
            ...
            const ruleDir = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'rules');
            try {
            ...
            try {
                const files = fs_1.default.readdirSync(ruleDir).filter(f => f.endsWith('.md'));
                if (files.length > 0) {
                    existingRules = files.join(', ');
                    // 读取每条规则的 frontmatter 摘要
                    existingRuleDetails = files.map(f => {
                        try {
                            const content = fs_1.default.readFileSync(path_1.default.join(ruleDir, f), 'utf8');
        ...
        if (!inst)
            inst = tiffaManager.getActive(); // 无 sessionId 时用项目级
        if (!inst)
            throw new Error('No active Tiffa instance');
    ...
    electron_1.ipcMain.handle('tiffa:activateSession', async (event, cwd, sessionId) => {
        try {
    ...
    // 关闭对话级实例（关闭标签时释放进程）
    electron_1.ipcMain.handle('tiffa:closeSession', async (event, cwd, sessionId) => {
        try {
    ...
    electron_1.ipcMain.handle('tiffa:abort', async (event, sessionId) => {
        ...
        inst.armAbortWatchdog((dead) => {
            ...
            if (key && tiffaManager.keyOf(dead) === key) {
                (0, session_utils_1.mainLog)(`[tiffa:abort] abort 未生效，强制关闭实例 key=${key}（会话可重开恢复）`);
                tiffaManager.closeByKey(key);
            }
            else {
                try {
    ...
    electron_1.ipcMain.handle('tiffa:setModel', async (event, provider, modelId, sessionId) => {
        ...
        if (!inst)
            throw new Error('No active Tiffa instance');
        try {
            const modelPath = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'current-model.json');
            fs_1.default.writeFileSync(modelPath, JSON.stringify({ provider, modelId, sessionId: sessionId || null, ts: Date.now() }, null, 2));
        }
        catch { }
        return inst.sendCommand({ type: 'set_model', provider, modelId });
    });
    electron_1.ipcMain.handle('tiffa:getModels', async (event, sessionId) => {
        const inst = _routeBySession(sessionId);
        if (!inst)
            throw new Error('No active Tiffa instance');
        return inst.sendCommand({ type: 'get_available_models' });
    });
    electron_1.ipcMain.handle('tiffa:isReady', async (event, sessionId) => {
        const inst = _routeBySession(sessionId);
        return inst ? inst.ready : false;
    });
    // 诊断：上报全部实例状态（吸收 dim 的 per-session 可观测性），
    // 解决"多个对话并行时无法判断某个后台内核是否还在跑"的问题。
    electron_1.ipcMain.handle('tiffa:diagnostics', async () => {
        const instances = [];
        for (const [key, inst] of tiffaManager.instances) {
            instances.push({
                key,
    ...
    electron_1.ipcMain.handle('tiffa:getState', async (event, sessionId) => {
        const inst = _routeBySession(sessionId);
        if (!inst)
            throw new Error('No active Tiffa instance');
        return inst.sendCommand({ type: 'get_state' });
    });
    electron_1.ipcMain.handle('tiffa:steer', async (event, message, sessionId) => {
    ...
    electron_1.ipcMain.handle('tiffa:followUp', async (event, message, sessionId) => {
        const inst = tiffaManager.resolve(tiffaManager.activeCwd, sessionId);
        if (!inst)
            throw new Error('no active process');
        inst.sendRaw({ type: 'follow_up', message });
    });
    electron_1.ipcMain.handle('tiffa:extensionResponse', async (event, id, value, sessionId) => {
        ...
        if (inst) {
            ...
            (0, session_utils_1.mainLog)(`[${inst._shortCwd()}#${inst.sessionId}] ui-resp id=${id} activeKey=${tiffaManager.activeKey}`);
            inst.sendRaw(frame);
        }
        else {
            (0, session_utils_1.mainLog)(`[ui-resp] id=${id} sessionId=${sessionId} INST NOT FOUND (routed to getActive)`);
        }
    });
    // 渲染层诊断日志（ask 抽屉等疑难问题排查）——落 data/logs/renderer.log，不影响 UI
    electron_1.ipcMain.on('renderer:log', (event, tag, msg) => {
        try {
            const filePath = path_1.default.join(global.PORTABLE_ROOT, 'data', 'logs', 'renderer.log');
            (0, session_utils_1.rotateLogIfNeeded)(filePath);
            const line = `[${new Date().toISOString()}] [${tag}] ${msg}\n`;
            fs_1.default.appendFileSync(filePath, line);
        }
        catch { }
    });
    electron_1.ipcMain.handle('tiffa:compact', async (event, sessionId) => {
        const inst = _routeBySession(sessionId);
        if (!inst)
            throw new Error('No active Tiffa instance');
        try {
    ...
    // ── 多实例管理 IPC ──
    electron_1.ipcMain.handle('tiffa:activate', async (event, cwd) => {
        try {
            const check = assertProjectCwd(cwd);
            if (!check.ok)
                return { error: check.error };
    ...
    electron_1.ipcMain.handle('tiffa:instances', async () => {
        return tiffaManager.getStatus();
    });
    // File system operations (for sidebar)
    electron_1.ipcMain.handle('fs:listDir', async (event, dirPath) => {
        try {
            const resolvedPath = path_1.default.resolve(dirPath || constants_1.currentWorkspaceDir);
            ...
            return entries.map(e => ({
                name: e.name,
                path: path_1.default.join(resolvedPath, e.name),
                isDirectory: e.isDirectory(),
                isFile: e.isFile(),
                size: e.isFile() ? fs_1.default.statSync(path_1.default.join(resolvedPath, e.name)).size : 0,
                ext: e.isFile() ? path_1.default.extname(e.name).toLowerCase() : '',
            })).sort((a, b) => {
    ...
    electron_1.ipcMain.handle('fs:writeFile', async (event, filePath, content) => {
        try {
            ...
            if (!resolvedPath.startsWith(constants_1.PORTABLE_ROOT)) {
    ...
    electron_1.ipcMain.handle('fetch:providerModels', async (event, baseUrl, apiKey) => {
        try {
    ...
    electron_1.ipcMain.handle('fs:readImage', async (event, filePath) => {
        try {
            const resolvedPath = path_1.default.resolve(filePath);
            const content = fs_1.default.readFileSync(resolvedPath);
            const ext = path_1.default.extname(resolvedPath).toLowerCase();
            const mimeMap = {
    ...
    electron_1.ipcMain.handle('custom:getStartupImage', () => (0, window_setup_1.syncCustomStartupImage)());
    // Shell operations
    ...
    // Path helpers
    electron_1.ipcMain.handle('path:workspace', async () => constants_1.currentWorkspaceDir);
    electron_1.ipcMain.handle('path:root', async () => constants_1.PORTABLE_ROOT);
    // ── XML Translation Toggle ──
    const XML_TRANSLATION_ENABLED_FILE = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'xml-translation-enabled');
    electron_1.ipcMain.handle('xml-translation:status', async () => {
        try {
            if (!fs_1.default.existsSync(XML_TRANSLATION_ENABLED_FILE))
                return { enabled: false };
            const content = fs_1.default.readFileSync(XML_TRANSLATION_ENABLED_FILE, 'utf8').trim();
    ...
    electron_1.ipcMain.handle('xml-translation:toggle', async (event, enabled) => {
        try {
            const dir = path_1.default.dirname(XML_TRANSLATION_ENABLED_FILE);
            if (!fs_1.default.existsSync(dir))
                fs_1.default.mkdirSync(dir, { recursive: true });
            fs_1.default.writeFileSync(XML_TRANSLATION_ENABLED_FILE, enabled ? 'true' : 'false', 'utf8');
    ...
    const COMPUTER_USE_ENABLED_FILE = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'computer-use-enabled');
    const COMPUTER_USE_MCP_JSON = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'mcp.json');
    // ── Computer Use v4：每应用执行策略（data/agent/computer-use-policies.json）──
    const COMPUTER_USE_POLICIES_FILE = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'computer-use-policies.json');
    function defaultPolicies() {
        return { default: 'ask', apps: {}, popup_ignore: ['Tiffa'] };
    }
    function readPolicies() {
        try {
            if (!fs_1.default.existsSync(COMPUTER_USE_POLICIES_FILE))
                return defaultPolicies();
            const cfg = JSON.parse(fs_1.default.readFileSync(COMPUTER_USE_POLICIES_FILE, 'utf8'));
    ...
    electron_1.ipcMain.handle('computer-use:policies:get', async () => readPolicies());
    electron_1.ipcMain.handle('computer-use:policies:set', async (event, cfg) => {
        try {
            const clean = {
            ...
            const dir = path_1.default.dirname(COMPUTER_USE_POLICIES_FILE);
            if (!fs_1.default.existsSync(dir))
                fs_1.default.mkdirSync(dir, { recursive: true });
            fs_1.default.writeFileSync(COMPUTER_USE_POLICIES_FILE, JSON.stringify(clean, null, 2) + '\n', 'utf8');
    ...
    const WINDOW_SNAPSHOT_CFG_FILE = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'window-snapshot.json');
    const DEFAULT_SNAPSHOT_HOTKEY = 'CommandOrControl+Alt+K';
    function readSnapshotCfg() {
        try {
            if (!fs_1.default.existsSync(WINDOW_SNAPSHOT_CFG_FILE))
                return { enabled: true, hotkey: DEFAULT_SNAPSHOT_HOTKEY };
            const cfg = JSON.parse(fs_1.default.readFileSync(WINDOW_SNAPSHOT_CFG_FILE, 'utf8'));
            return {
    ...
    electron_1.ipcMain.handle('window-snapshot:getHotkey', async () => readSnapshotCfg());
    electron_1.ipcMain.handle('window-snapshot:setHotkey', async (event, cfg) => {
        try {
            const clean = {
                enabled: cfg && cfg.enabled !== false,
                hotkey: cfg && typeof cfg.hotkey === 'string' && cfg.hotkey ? cfg.hotkey : DEFAULT_SNAPSHOT_HOTKEY,
            };
            const dir = path_1.default.dirname(WINDOW_SNAPSHOT_CFG_FILE);
            if (!fs_1.default.existsSync(dir))
                fs_1.default.mkdirSync(dir, { recursive: true });
            fs_1.default.writeFileSync(WINDOW_SNAPSHOT_CFG_FILE, JSON.stringify(clean, null, 2) + '\n', 'utf8');
    ...
    async function captureWindowSnapshot() {
        try {
            ...
            if (!sources || sources.length === 0) {
                (0, session_utils_1.mainLog)('[主进程] captureWindowSnapshot: 无可用窗口源');
            ...
            if (!img || img.isEmpty()) {
                (0, session_utils_1.mainLog)('[主进程] captureWindowSnapshot: 快照为空');
                if (mainWindow && !mainWindow.isDestroyed()) {
            ...
            (0, session_utils_1.mainLog)(`[主进程] captureWindowSnapshot: 已捕获 ${src.name} (${(b64.length * 0.75 / 1024).toFixed(0)}KB)`);
            if (mainWindow && !mainWindow.isDestroyed()) {
            ...
            (0, session_utils_1.mainLog)(`[主进程] captureWindowSnapshot 失败: ${String((err && err.message) || err)}`);
            if (mainWindow && !mainWindow.isDestroyed()) {
    ...
    function registerSnapshotHotkey() {
        try {
            ...
            if (!cfg.enabled) {
                (0, session_utils_1.mainLog)('[主进程] 窗口快照热键: 已禁用，不注册');
                return;
            }
            const ok = electron_1.globalShortcut.register(cfg.hotkey, () => {
                void captureWindowSnapshot();
            });
            (0, session_utils_1.mainLog)(`[主进程] 窗口快照热键 ${cfg.hotkey}: ${ok ? 'OK' : '注册失败（可能被占用）'}`);
            console.log(`[主进程] 窗口快照热键 ${cfg.hotkey}: ${ok ? 'OK' : '注册失败（可能被占用）'}`);
        }
        catch (err) {
            (0, session_utils_1.mainLog)(`[主进程] registerSnapshotHotkey 失败: ${err.message}`);
            console.error('[主进程] registerSnapshotHotkey 失败:', err.message);
        }
    }
    registerSnapshotHotkey();
    electron_1.ipcMain.handle('window-snapshot:reload', async () => {
        registerSnapshotHotkey();
        return { ok: true };
    });
    function isComputerUseEnabled() {
        try {
            if (!fs_1.default.existsSync(COMPUTER_USE_ENABLED_FILE))
                return false; // 默认关，启动快
            return fs_1.default.readFileSync(COMPUTER_USE_ENABLED_FILE, 'utf8').trim() === 'true';
        }
        catch {
            return false;
        }
    ...
    electron_1.ipcMain.handle('computer-use:status', async () => ({ enabled: isComputerUseEnabled() }));
    electron_1.ipcMain.handle('computer-use:toggle', async (event, enabled) => {
        try {
            const dir = path_1.default.dirname(COMPUTER_USE_ENABLED_FILE);
            if (!fs_1.default.existsSync(dir))
                fs_1.default.mkdirSync(dir, { recursive: true });
            fs_1.default.writeFileSync(COMPUTER_USE_ENABLED_FILE, enabled ? 'true' : 'false', 'utf8');
    ...
    electron_1.ipcMain.handle('models:read', async () => {
        try {
            if (!fs_1.default.existsSync(MODELS_YML)) {
                return { error: 'models.yml not found' };
            }
            const raw = fs_1.default.readFileSync(MODELS_YML, 'utf8');
    ...
    electron_1.ipcMain.handle('models:restart', async () => {
        try {
            // 重启所有实例（模型配置变更后）
    ...
    electron_1.ipcMain.handle('models:writeProvider', async (event, providerId, cfg) => {
        try {
            ...
            if (fs_1.default.existsSync(MODELS_YML)) {
                const raw = fs_1.default.readFileSync(MODELS_YML, 'utf8');
                doc = (0, yaml_1.parseDocument)(raw);
                if (doc.errors.length > 0) {
                    return { error: `models.yml 解析失败（${doc.errors[0].message}），为避免破坏原文件已中止写入` };
                }
            }
            else {
                doc = new yaml_1.Document({});
            }
            doc.setIn(['providers', providerId], doc.createNode(clean));
            fs_1.default.mkdirSync(path_1.default.dirname(MODELS_YML), { recursive: true });
            fs_1.default.writeFileSync(MODELS_YML, doc.toString(), 'utf8');
    ...
    electron_1.ipcMain.handle('models:deleteProvider', async (event, providerId) => {
        try {
            if (!fs_1.default.existsSync(MODELS_YML))
                return { success: true };
            const raw = fs_1.default.readFileSync(MODELS_YML, 'utf8');
    ...
    electron_1.ipcMain.handle('config:writeApprovalMode', async (event, tiffaMode) => {
        try {
            const agentMode = TIFFA_APPROVAL_MODE_MAP[tiffaMode] || 'yolo';
            let doc;
            if (fs_1.default.existsSync(CONFIG_YML)) {
                const raw = fs_1.default.readFileSync(CONFIG_YML, 'utf8');
                doc = (0, yaml_1.parseDocument)(raw);
            }
            else {
                doc = new yaml_1.Document();
     ...
     */
    function _syncGoalArmSessionId(inst, armedWith) {
        if (!inst || !armedWith)
            return;
        let checks = 0;
        const timer = setInterval(() => {
            ...
            if (now && now !== armedWith) {
                ...
                if (arm.enabled && arm.sessionId === armedWith) {
                    (0, goal_mode_1.writeGoalArm)({ ...arm, sessionId: now });
    ...
    async function _armAndStart(text, budget, sessionId, autoResume) {
        ...
        const existing = (0, goal_mode_1.readGoalState)(inst.sessionId || sessionId);
    ...
    electron_1.ipcMain.handle('goal:draft', async (event, request, sessionId) => {
    ...
    // 打开文件夹选择器
    electron_1.ipcMain.handle('workspace:openFolderDialog', async () => {
        try {
            const result = await electron_1.dialog.showOpenDialog(mainWindow, {
                title: '选择项目文件夹',
                defaultPath: constants_1.currentWorkspaceDir,
            ...
            if (path_1.default.resolve(selected) === constants_1.DEFAULT_WORKSPACE_DIR) {
    ...
    // 切换工作区（懒启动新实例或复用已有实例）
    electron_1.ipcMain.handle('workspace:change', async (event, newCwd) => {
        try {
            if (!newCwd) {
                return { error: '路径为空' };
            }
            let resolved = path_1.default.resolve(newCwd);
            // 显式用户操作：如果路径曾被删除，从黑名单移除（允许重新添加）
            unremoveCwd(resolved);
            // workspace 下的项目如果目录不存在，自动创建
            if (!fs_1.default.existsSync(resolved)) {
                const wsSuffix = (0, session_utils_1.extractWorkspaceSuffix)(resolved);
                if (wsSuffix) {
    ...
    const SESSIONS_DIR = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'sessions');
    const ARCHIVE_DIR = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'sessions-archive');
    const PROJECTS_JSON = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'projects.json');
    const REMOVED_CWDS_FILE = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'removed-cwds.json');
    function readRemovedCwds() {
        try {
            if (fs_1.default.existsSync(REMOVED_CWDS_FILE))
                return JSON.parse(fs_1.default.readFileSync(REMOVED_CWDS_FILE, 'utf8'));
        }
        catch { }
        return [];
    }
    function writeRemovedCwds(list) {
        fs_1.default.writeFileSync(REMOVED_CWDS_FILE, JSON.stringify(list), 'utf8');
    }
    ...
    function readProjectsJson() {
        try {
            if (fs_1.default.existsSync(PROJECTS_JSON)) {
                const raw = fs_1.default.readFileSync(PROJECTS_JSON, 'utf8');
    ...
    function writeProjectsJson(projects) {
        try {
            fs_1.default.writeFileSync(PROJECTS_JSON, JSON.stringify({ projects }, null, 2), 'utf8');
        }
        catch { }
    }
    ...
    function ensureProjectInJson(cwd) {
        ...
        if (!fs_1.default.existsSync(normalized)) {
            if ((0, session_utils_1.extractWorkspaceSuffix)(normalized)) {
                ...
                const sessionDir = path_1.default.join(SESSIONS_DIR, sessionDirName);
    ...
    function encodeSessionDirName(cwdPath) {
        return (0, session_utils_1.stableSessionDirName)(cwdPath);
    }
    ...
    function migrateSessionsToProjectsJson() {
        ...
        const existingCwds = new Set(existing.map(p => path_1.default.resolve(p.cwd)));
        let changed = false;
        if (fs_1.default.existsSync(SESSIONS_DIR)) {
            let dirs = [];
            try {
                dirs = fs_1.default.readdirSync(SESSIONS_DIR, { withFileTypes: true }).filter(d => d.isDirectory());
            }
            catch (err) {
            ...
            for (const dir of dirs) {
                ...
                const dirPath = path_1.default.join(SESSIONS_DIR, dir.name);
                ...
                let normalized = path_1.default.resolve(cwd);
                // 旧盘符迁移：如果 cwd 不在当前 PORTABLE_ROOT 下但属于某个 workspace，
                // 迁移到当前 PORTABLE_ROOT 的 workspace 下（换电脑/换盘符场景）
                const wsSuffix = (0, session_utils_1.extractWorkspaceSuffix)(normalized);
                if (wsSuffix) {
                    const migrated = path_1.default.resolve(path_1.default.join(path_1.default.join(constants_1.PORTABLE_ROOT, 'workspace'), wsSuffix));
    ...
    function migrateSessionDirsForNewRoot() {
        if (!fs_1.default.existsSync(SESSIONS_DIR))
            return;
        const currentWorkspace = path_1.default.join(constants_1.PORTABLE_ROOT, 'workspace');
        const projects = readProjectsJson();
        const existingCwds = new Set(projects.map(p => path_1.default.resolve(p.cwd)));
        let changed = false;
        // 步骤1: 修复 projects.json 中旧盘符的条目
        const newProjects = [];
        for (const proj of projects) {
            const resolved = path_1.default.resolve(proj.cwd);
            ...
            const workspaceSuffix = (0, session_utils_1.extractWorkspaceSuffix)(resolved);
            if (workspaceSuffix) {
                const newCwd = path_1.default.join(currentWorkspace, workspaceSuffix);
        ...
        for (const proj of newProjects) {
            const resolved = path_1.default.resolve(proj.cwd).toLowerCase();
        ...
        for (const dir of dirs) {
            ...
            const workspaceSuffix = (0, session_utils_1.extractWorkspaceSuffix)(oldCwd);
            if (!workspaceSuffix) {
            ...
            if (isRemovedCwd(path_1.default.resolve(newCwd))) {
            ...
            const newDirPath = path_1.default.join(SESSIONS_DIR, newDirName);
            if (fs_1.default.existsSync(newDirPath)) {
                ...
                const oldFiles = fs_1.default.readdirSync(dirPath);
                for (const f of oldFiles) {
                    const src = path_1.default.join(dirPath, f);
                    const dst = path_1.default.join(newDirPath, f);
                ...
                try {
                    fs_1.default.renameSync(dirPath, newDirPath);
                }
                catch (err) {
            ...
            if (!existingCwds.has(path_1.default.resolve(newCwd))) {
                projects.push({
                    cwd: newCwd,
                ...
                existingCwds.add(path_1.default.resolve(newCwd));
                changed = true;
            }
        }
        if (changed)
    ...
    ensureProjectInJson(constants_1.currentWorkspaceDir);
    ...
    function parseSessionHeader(filePath) {
        try {
            ...
            return { path: filePath, name: path_1.default.basename(filePath), error: err.message };
        }
    }
    // ── 自动生成会话标题 ──
    ...
    electron_1.ipcMain.handle('sessions:listProjects', async () => {
        try {
            ...
            const result = [];
            for (const proj of projects) {
                const normalized = path_1.default.resolve(proj.cwd);
                const dirName = encodeSessionDirName(normalized);
                const projectPath = path_1.default.join(SESSIONS_DIR, dirName);
                ...
                const isAttachmentDir = (dirName) => {
                    if (!_topNames) {
                        _topNames = new Set();
                        try {
                            for (const e of fs_1.default.readdirSync(projectPath, { withFileTypes: true })) {
                                if (e.isFile() && e.name.endsWith('.jsonl'))
                                    _topNames.add(e.name.slice(0, -'.jsonl'.length));
                            }
                        }
                        catch { /* ignore */ }
                    }
                    for (const t of _topNames)
                        if (dirName === t || dirName.startsWith(t + '_'))
                            return true;
                    return false;
                };
                ...
                const countJsonl = (dir, isTop) => {
                    ...
                    for (const entry of entries) {
                        ...
                        if (entry.isDirectory()) {
                            ...
                            // 内部 jsonl 不是独立会话（否则一个对话的归档会分裂成多个列表项）
                            if (isTop && isAttachmentDir(entry.name))
                                continue;
            ...
            for (const proj of result) {
                try {
                    const projectPath = path_1.default.join(SESSIONS_DIR, proj.dirName);
                    if (fs_1.default.existsSync(projectPath)) {
    ...
    electron_1.ipcMain.handle('sessions:listSessions', async (event, projectDirName) => {
        try {
            const projectPath = path_1.default.join(SESSIONS_DIR, projectDirName);
            if (!fs_1.default.existsSync(projectPath))
                return [];
            ...
            const isAttachmentDir = (dirName) => {
                ...
                for (const e of fs_1.default.readdirSync(projectPath, { withFileTypes: true })) {
                    if (e.isFile() && e.name.endsWith('.jsonl'))
                        topNames.add(e.name.slice(0, -'.jsonl'.length));
                }
            ...
            files.sort((a, b) => path_1.default.basename(a).localeCompare(path_1.default.basename(b)));
            const sessions = [];
    ...
    electron_1.ipcMain.handle('sessions:switch', async (event, sessionPath) => {
        return _active().sendCommand({ type: 'switch_session', sessionPath });
    });
    electron_1.ipcMain.handle('sessions:new', async () => {
        return _active().sendCommand({ type: 'new_session' });
    });
    // ── Session History Loading ──
    electron_1.ipcMain.handle('sessions:loadHistory', async (event, sessionPath, opts) => {
        try {
            const resolvedPath = path_1.default.resolve(sessionPath);
            if (!resolvedPath.endsWith('.jsonl') || !fs_1.default.existsSync(resolvedPath)) {
            ...
            if (wantAll) {
                const text = await fs_1.default.promises.readFile(resolvedPath, 'utf8');
                lines = text.split('\n');
                reachedStart = true;
            }
            else {
                const r = await (0, session_utils_1.readTailLines)(resolvedPath, skip + tail);
                lines = r.lines;
                reachedStart = r.reachedStart;
                droppedAny = r.droppedAny;
            }
            const all = (0, session_utils_1.parseSessionLines)(lines);
    ...
    function sessionFileBelongsToCwd(filePath, cwdLower) {
        try {
            ...
            return header && header.cwd && path_1.default.resolve(header.cwd).toLowerCase() === cwdLower;
        }
        catch {
            return false;
        }
    ...
    electron_1.ipcMain.handle('sessions:archiveProject', async (event, dirName, cwd) => {
        try {
            ...
            if (cwd) {
                const normalized = path_1.default.resolve(cwd);
                project = allProjects.find(p => path_1.default.resolve(p.cwd) === normalized) || null;
            ...
            const srcDir = path_1.default.join(SESSIONS_DIR, dirName);
            if (fs_1.default.existsSync(srcDir)) {
                const projectCwd = (project ? path_1.default.resolve(project.cwd) : null) || (cwd ? path_1.default.resolve(cwd) : null);
                // 编码碰撞检测：是否还有活跃项目共享此会话目录
                const hasSibling = projectCwd && allProjects.some(p => !p.archived && path_1.default.resolve(p.cwd) !== projectCwd &&
                    encodeSessionDirName(path_1.default.resolve(p.cwd)) === dirName);
                if (!fs_1.default.existsSync(ARCHIVE_DIR))
                    fs_1.default.mkdirSync(ARCHIVE_DIR, { recursive: true });
                const destDir = path_1.default.join(ARCHIVE_DIR, dirName);
                if (hasSibling) {
                    ...
                    if (fs_1.default.existsSync(finalDest)) {
                        finalDest = path_1.default.join(ARCHIVE_DIR, dirName + '-' + Date.now());
                    }
                    fs_1.default.renameSync(srcDir, finalDest);
                }
            }
            return { success: true };
        }
    ...
    electron_1.ipcMain.handle('sessions:deleteProject', async (event, dirName, cwd) => {
        try {
            // 定位项目（优先按 cwd 精确匹配：编码有损，logo\design 与 logo-design 的 dirName 相同）
            const allProjects = readProjectsJson();
            let project = null, normalized = null;
            if (cwd) {
                normalized = path_1.default.resolve(cwd);
                project = allProjects.find(p => path_1.default.resolve(p.cwd) === normalized) || null;
            ...
            if (normalized) {
                const filtered = allProjects.filter(p => path_1.default.resolve(p.cwd) !== normalized);
                writeProjectsJson(filtered);
                // 碰撞检测：删除后是否仍有其他项目共享同一会话目录名
                hasSibling = filtered.some(p => encodeSessionDirName(path_1.default.resolve(p.cwd)) === dirName);
            }
            const projectCwd = normalized || ((project && project.cwd) ? path_1.default.resolve(project.cwd) : null);
            ...
            const srcDir = path_1.default.join(SESSIONS_DIR, dirName);
            const archiveSrcDir = path_1.default.join(ARCHIVE_DIR, dirName);
            ...
            if (projectCwd) {
                ...
                const wsSuffix = (0, session_utils_1.extractWorkspaceSuffix)(projectCwd);
                if (wsSuffix) {
                    // 嵌套项目保护：目录内还有其他注册项目时不删物理目录
                    // （否则 ensureProjectInJson 会因嵌套项目的会话记录立即重建父目录）
                    const prefix = projectCwd.toLowerCase() + path_1.default.sep;
                    const hasNested = readProjectsJson().some(p => path_1.default.resolve(p.cwd).toLowerCase().startsWith(prefix));
                    if (hasNested) {
    ...
    electron_1.ipcMain.handle('sessions:listArchived', async () => {
        try {
    ...
    electron_1.ipcMain.handle('sessions:restoreProject', async (event, dirName) => {
        try {
            ...
            // 移动归档目录回 sessions 区
            const srcDir = path_1.default.join(ARCHIVE_DIR, dirName);
            if (fs_1.default.existsSync(srcDir)) {
                const destDir = path_1.default.join(SESSIONS_DIR, dirName);
                if (!fs_1.default.existsSync(destDir)) {
    ...
    electron_1.ipcMain.handle('sessions:archiveSession', async (event, sessionPath) => {
        try {
            const resolved = path_1.default.resolve(sessionPath);
            if (!resolved.endsWith('.jsonl') || !fs_1.default.existsSync(resolved)) {
                return { error: 'Session file not found' };
            ...
            // 从文件路径反推项目 dirName
            const sessionDir = path_1.default.dirname(resolved);
            const dirName = path_1.default.basename(sessionDir);
            const archiveProjectDir = path_1.default.join(ARCHIVE_DIR, dirName);
            if (!fs_1.default.existsSync(archiveProjectDir)) {
                fs_1.default.mkdirSync(archiveProjectDir, { recursive: true });
            }
            const destPath = path_1.default.join(archiveProjectDir, path_1.default.basename(resolved));
            // 目标已存在时加时间戳防冲突
            let finalDest = destPath;
            if (fs_1.default.existsSync(finalDest)) {
                finalDest = path_1.default.join(archiveProjectDir, path_1.default.basename(resolved, '.jsonl') + '-' + Date.now() + '.jsonl');
            }
            fs_1.default.renameSync(resolved, finalDest);
            return { success: true };
        }
        catch (err) {
            return { error: err.message };
    ...
    electron_1.ipcMain.handle('sessions:deleteSession', async (event, sessionPath) => {
        try {
            const resolved = path_1.default.resolve(sessionPath);
    ...
    electron_1.ipcMain.handle('sessions:rename', async (event, sessionPath, newTitle) => {
        try {
            const resolved = path_1.default.resolve(sessionPath);
            if (!resolved.endsWith('.jsonl') || !fs_1.default.existsSync(resolved)) {
                return { error: 'Session file not found' };
            ...
            const sid = (0, session_utils_1.extractSessionIdFromPath)(resolved);
            const inst = sid
            ...
            if (inst) {
                try {
                    ...
                    console.warn(`[rename] set_session_name 失败，降级追加 title 行: ${err.message}`);
                }
            }
            ...
            fs_1.default.appendFileSync(resolved, titleLine, 'utf8');
            return { success: true };
        }
        catch (err) {
            return { error: err.message };
    ...
    async function callCompletion(baseUrl, model, apiKey, prompt, maxTokens) {
        ...
        try {
            ...
            if (isDoubao)
                body.thinking = { type: 'disabled' }; // 仅豆包需要关思考（实测 16s→1.9s）
    ...
    electron_1.ipcMain.handle('ai:complete', async (event, { prompt, maxTokens, providerHint, modelHint }) => {
        const candidates = [];
        const seen = new Set();
        const push = (c) => {
        ...
        try {
            const bpPath = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'bypass-model.json');
            if (fs_1.default.existsSync(bpPath)) {
                const bp = JSON.parse(fs_1.default.readFileSync(bpPath, 'utf8'));
        ...
        try {
            const cfgPath = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'managed-skills', 'computer-use', 'grounding.json');
        ...
        try {
            const data = js_yaml_1.default.load(fs_1.default.readFileSync(path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'models.yml'), 'utf8'));
            const providers = data && data.providers;
            if (providers) {
    ...
    electron_1.ipcMain.handle('settings:getBypassModel', async () => {
        try {
            const p = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'bypass-model.json');
            if (!fs_1.default.existsSync(p))
                return { enabled: false, baseUrl: '', apiKey: '', model: '', ts: 0 };
    ...
    electron_1.ipcMain.handle('settings:saveBypassModel', async (event, cfg) => {
        try {
            const p = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'bypass-model.json');
            const clean = {
    ...
    electron_1.ipcMain.handle('settings:getGroundingModel', async () => {
        try {
            const p = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'managed-skills', 'computer-use', 'grounding.json');
            if (!fs_1.default.existsSync(p))
                return { enabled: false, api_base: '', api_key: '', model: '' };
            return JSON.parse(fs_1.default.readFileSync(p, 'utf8'));
        }
        catch (err) {
            return { error: err.message };
    ...
    electron_1.ipcMain.handle('settings:saveGroundingModel', async (event, cfg) => {
        try {
            const p = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'agent', 'managed-skills', 'computer-use', 'grounding.json');
            const clean = {
    ...
    electron_1.ipcMain.handle('settings:checkModelHealth', async (event, arg) => {
    ...
    // ── 列出归档的会话（单会话级别）
    electron_1.ipcMain.handle('sessions:listArchivedSessions', async (event, projectDirName) => {
        try {
            const archiveProjectDir = path_1.default.join(ARCHIVE_DIR, projectDirName);
            if (!fs_1.default.existsSync(archiveProjectDir))
                return [];
            const files = fs_1.default.readdirSync(archiveProjectDir)
                .filter(f => f.endsWith('.jsonl'))
                .sort();
            const sessions = [];
            for (const file of files) {
                sessions.push(parseSessionHeader(path_1.default.join(archiveProjectDir, file)));
            }
            return sessions;
        }
        catch (err) {
    ...
    // ── 恢复归档的会话 ──
    electron_1.ipcMain.handle('sessions:restoreSession', async (event, sessionPath) => {
        try {
            const resolved = path_1.default.resolve(sessionPath);
            if (!resolved.endsWith('.jsonl') || !fs_1.default.existsSync(resolved)) {
                return { error: 'Session file not found' };
            ...
            // 从归档路径反推项目 dirName
            const archiveDir = path_1.default.dirname(resolved);
            const dirName = path_1.default.basename(archiveDir);
            const activeProjectDir = path_1.default.join(SESSIONS_DIR, dirName);
            if (!fs_1.default.existsSync(activeProjectDir)) {
                fs_1.default.mkdirSync(activeProjectDir, { recursive: true });
            }
            const destPath = path_1.default.join(activeProjectDir, path_1.default.basename(resolved));
            let finalDest = destPath;
            if (fs_1.default.existsSync(finalDest)) {
                finalDest = path_1.default.join(activeProjectDir, path_1.default.basename(resolved, '.jsonl') + '-' + Date.now() + '.jsonl');
            }
            fs_1.default.renameSync(resolved, finalDest);
    ...
    // ── 永久删除归档的会话（单会话级别，幂等）──
    electron_1.ipcMain.handle('sessions:deleteArchivedSession', async (event, sessionPath) => {
        try {
            const resolved = path_1.default.resolve(sessionPath);
            if (!resolved.endsWith('.jsonl')) {
                return { error: 'Session file not found' };
    ...
    // ── 读取用户消息列表（用于分支功能） ──
    electron_1.ipcMain.handle('sessions:getUserEntries', async (event, sessionPath) => {
        try {
            const resolved = path_1.default.resolve(sessionPath);
            if (!resolved.endsWith('.jsonl') || !fs_1.default.existsSync(resolved)) {
                return { error: 'Session file not found' };
            ...
            for (const line of lines) {
                try {
                    ...
                    if (obj.type === 'message' && obj.message && obj.message.role === 'user') {
                        ...
                        if (text)
                            entries.push({ id: obj.message.id || String(entries.length), text: text.substring(0, 200) });
                    }
                }
                catch { }
            }
    ...
    // ── 导出会话为 HTML ──
    electron_1.ipcMain.handle('sessions:exportHtml', async (event, sessionPath) => {
        try {
            const resolved = path_1.default.resolve(sessionPath);
            if (!resolved.endsWith('.jsonl') || !fs_1.default.existsSync(resolved)) {
                return { error: 'Session file not found' };
            ...
            const desktopPath = path_1.default.join(require('os').homedir(), 'Desktop');
            const sessionName = path_1.default.basename(resolved, '.jsonl').substring(0, 30);
            const exportPath = path_1.default.join(desktopPath, `对话导出-${sessionName}-${Date.now()}.html`);
            fs_1.default.writeFileSync(exportPath, htmlParts.join('\n'), 'utf8');
    ...
    electron_1.ipcMain.handle('sessions:getRemovedCwds', async () => readRemovedCwds());
    electron_1.ipcMain.handle('sessions:addRemovedCwd', async (event, cwd) => {
        const list = readRemovedCwds();
        const normalized = path_1.default.resolve(cwd).toLowerCase();
    ...
    electron_1.ipcMain.handle('sessions:removeRemovedCwd', async (event, cwd) => {
        const list = readRemovedCwds();
        const normalized = path_1.default.resolve(cwd).toLowerCase();
    ...
    electron_1.ipcMain.handle('memory:recall', async (event, query) => {
        const q = (query || '').trim();
        if (!q)
            return { results: [], error: '空查询' };
        try {
            const bunExe = path_1.default.join(constants_1.PORTABLE_ROOT, 'npm-global', 'node_modules', 'bun', 'bin', 'bun.exe');
            const cliScript = path_1.default.join(constants_1.AGENT_DIR, 'mcp-servers', 'wide-recall.ts');
            const { execFileSync } = require('child_process');
            const output = execFileSync(bunExe, [cliScript, '--query', q, '--limit', '20'], {
                encoding: 'utf8',
                timeout: 60000,
                maxBuffer: 5 * 1024 * 1024,
                env: { ...process.env, PI_CODING_AGENT_DIR: constants_1.AGENT_DIR, PORTABLE_ROOT: constants_1.PORTABLE_ROOT },
            });
    ...
    const MEMORY_DIR = path_1.default.join(constants_1.PORTABLE_ROOT, 'data', 'memory');
    const AI_MD = path_1.default.join(MEMORY_DIR, 'AI.md');
    const USER_MD = path_1.default.join(MEMORY_DIR, 'USER.md');
    ...
    electron_1.ipcMain.handle('memory:getIdentity', async () => {
        try {
            const aiContent = fs_1.default.existsSync(AI_MD) ? fs_1.default.readFileSync(AI_MD, 'utf8') : '';
            const userContent = fs_1.default.existsSync(USER_MD) ? fs_1.default.readFileSync(USER_MD, 'utf8') : '';
    ...
    electron_1.ipcMain.handle('memory:saveIdentity', async (event, aiName, userName, gender, persona) => {
        try {
            ...
            if (!name && !title)
                return { ok: false, error: '名字与称呼不能都为空' };
            // 写 AI.md（整体覆盖，结构固定；未显式更新的性别/角色卡保留旧值）
            if (name) {
                const prevAi = fs_1.default.existsSync(AI_MD) ? fs_1.default.readFileSync(AI_MD, 'utf8') : '';
                ...
                fs_1.default.writeFileSync(AI_MD, aiMd, 'utf8');
            }
            // 更新 USER.md 的「称呼」字段（保留其余内容）
            if (title) {
                let userContent = fs_1.default.existsSync(USER_MD) ? fs_1.default.readFileSync(USER_MD, 'utf8') : '# 用户档案\n\n- 称呼：\n';
                ...
                fs_1.default.writeFileSync(USER_MD, userContent, 'utf8');
            }
            return { ok: true };
        }
        catch (err) {
...
function healKernelExtensionHandlerTimeout() {
    try {
        const cliJs = path_1.default.join(constants_1.PORTABLE_ROOT, 'npm-global', 'node_modules', '@oh-my-pi', 'pi-coding-agent', 'dist', 'cli.js');
        if (!fs_1.default.existsSync(cliJs))
            return;
        const m = '/*TIFFA_EXT_HANDLER_TIMEOUT_120000*/';
        let src = fs_1.default.readFileSync(cliJs, 'utf8');
        if (src.includes(m))
            return; // 已打
        const anchor = 'var Xle=30000,';
        const idx = src.indexOf(anchor);
        if (idx < 0) {
            console.warn('[kernel-heal] extension-handler-timeout 锚点未找到（内核版本可能变化），跳过');
            return;
        }
        const newSrc = src.slice(0, idx) + 'var Xle=120000' + m + ',' + src.slice(idx + anchor.length);
        fs_1.default.writeFileSync(cliJs, newSrc, 'utf8');
        (0, session_utils_1.mainLog)('[kernel-heal] 内核扩展 handler 超时 30s -> 120s（session_before_compact 旁路总结需要更长窗口）');
    }
    catch (e) {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { TiffaInstance: tiffa_instance_1.TiffaInstance, TiffaInstanceManager: tiffa_manager_1.TiffaInstanceManager, tiffaManager, readTailLines: session_utils_1.readTailLines, parseSessionLines: session_utils_1.parseSessionLines };
}
