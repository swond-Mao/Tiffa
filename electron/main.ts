...
function setupIpc() {
   ...
   */
  const _goalDraftRestore = new Map<string, { provider: string; modelId: string }>();

  /** 把会话模型切回转写前的那个（幂等：没有借调记录就直接返回） */
  async function _restoreDraftModel(sessionId?: string | null): Promise<void> {
    const key = sessionId || '';
  ...
  async function _resolveGoalInstance(sessionId) {
    ...
    if (!inst) inst = tiffaManager.getActive();
    if (inst && !inst.ready && sessionId) {
      await new Promise((resolve) => {
        let checks = 0;
        const check = setInterval(() => {
          checks++;
          if (inst.ready || checks > 150) { clearInterval(check); resolve(null); }
        }, 100);
        if (inst.process) inst.process.once('exit', () => { clearInterval(check); resolve(null); });
      });
    }
  ...
  ipcMain.handle('goal:status', async (event, sessionId) => {
    ...
    return {
      ...
      resume: readGoalResume(sessionId),
    };
  });

  /** 前端传来的续跑配置做归一化 + 兜底上限：不设上限的续跑=放任烧 token */
  function normalizeAutoResume(raw: any): any {
    if (!raw || typeof raw !== 'object' || raw.enabled !== true) return null;
    const n = (v: unknown, def: number, cap: number) => {
   ...
   */
  async function _armAndStart(text: string, budget: number | null, sessionId?: string | null, autoResume?: any) {
    ...
    clearGoalResume(armedWith || sessionId);
    ...
    clearGoalDraft(armedWith || sessionId);
    writeGoalArm({ enabled: true, objective: text, tokenBudget: budget, sessionId: armedWith, autoResume: normalizeAutoResume(autoResume) });
    const api = await _goalModelApi(inst);
    const forced = isForceCapable(api);
    try {
      await inst.sendCommand({ type: 'prompt', message: forced ? buildCreateCommand(text, budget) : buildSoftCreateMessage(text) });
    } catch (err) {
  ...
  ipcMain.handle('goal:start', async (event, objective, tokenBudget, sessionId, autoResume) => {
  ...
  ipcMain.handle('goal:draft', async (event, request, sessionId) => {
    ...
    const running = readGoalState(inst.sessionId || sessionId);
    ...
    if (inst.isStale) {
      console.log(
        `[主进程] 内核已 ${Math.round((Date.now() - inst.lastActiveTime) / 1000)} 秒无事件，状态标志不可信，按空闲放行转写`,
      );
    }
    if (!inst.ready) {
    ...
    let model: { provider: string; modelId: string; api: string } | undefined;
    try {
      const st: any = await inst.sendCommand({ type: 'get_state' });
    ...
    const draftModel = readGoalDraftModel();
    if (draftModel && model && (draftModel.provider !== model.provider || draftModel.modelId !== model.modelId)) {
      try {
        ...
        console.log(
          `[主进程] 转写临时切模型 ${model.provider}/${model.modelId} → ${draftModel.provider}/${draftModel.modelId}`,
        );
      } catch (e: any) {
    ...
    if (sid) {
      ...
      const timer = setInterval(() => {
        ...
        const now = inst.sessionId;
        if (now && now !== curSid) {
          const d = readGoalDraft(curSid) || readGoalDraft(null);
          if (d && d.status === 'pending') {
            // 旧 id 也记进别名：迁移可能发生多次（临时 UUID → 真实 id），别名保证任一方都能找回
            clearGoalDraft(curSid);
            writeGoalDraft({ ...d, sessionId: now, aliases: [...(d.aliases ?? []), curSid, now] });
            console.log(`[主进程] 目标草稿会话 id 跟随迁移 ${curSid} → ${now}`);
          }
          curSid = now;
        }
        if (checks > 3000 || !inst.process) clearInterval(timer); // 最长跟 5 分钟
      }, 100);
    }
    console.log(
      `[主进程] 目标草稿已下发 session=${sid} model=${model ? `${model.provider}/${model.modelId}` : '未知'}`,
    );
    ...
    setTimeout(() => {
      try {
        const cur = readGoalDraft(inst.sessionId || sid) || readGoalDraft(sid);
        if (!cur || cur.status !== 'pending' || cur.ts !== draftTs) return; // 已产出/已放弃/换了一轮
        const diag = {
        ...
        writeGoalDraft({ ...cur, diag });
        console.log(
          `[主进程] 转写诊断+${diag.atSec}s invoked=${diag.invoked} idle=${diag.idleSec}s asks=${diag.pendingAsks}`,
        );
      } catch (e: any) {
  ...
  ipcMain.handle('goal:draftStatus', async (event, sessionId) => {
    ...
    if (draft && draft.status !== 'pending') await _restoreDraftModel(sessionId);
    return { draft: draft || null };
  });

  ipcMain.handle('goal:draftApply', async (event, draft, tokenBudget, sessionId, autoResume) => {
  ...
  // ── 转写专用模型（可空 = 跟随当前会话的模型）──
  ipcMain.handle('settings:getGoalDraftModel', async () => readGoalDraftModel());
  ipcMain.handle('settings:saveGoalDraftModel', async (_event, cfg) => {
    const provider = String(cfg?.provider ?? '').trim();
    const modelId = String(cfg?.modelId ?? '').trim();
    writeGoalDraftModel(provider && modelId ? { provider, modelId } : null);
  ...
  ipcMain.handle('goal:stop', async (event, op, sessionId) => {
    ...
    const inst = await _resolveGoalInstance(sessionId);
    if (!inst) return { ok: false, error: '没有可用会话实例' };
    // 先撤掉武装：即使模型没执行收尾，也不会再有新的目标被创建
    // autoResume 一并撤掉 —— 否则「结束目标」后外挂还在 agent_end 里接着起下一回合
    writeGoalArm({ ...readGoalArm(), enabled: false, autoResume: null });
    clearGoalResume(sessionId);
    const api = await _goalModelApi(inst);
    if (!isForceCapable(api)) {
      return {
        ok: false,
    ...
    try {
      await inst.sendCommand({ type: 'prompt', message: buildCloseCommand(kind) });
    } catch (err) {
      return { ok: false, error: `发送失败：${err.message}` };
    }
