# Tiffa

> 中文文档如下 · [English](README.en.md)

**与万物对弈，伴时间同行** — 一款**完全便携、绝对隐私**的本地 AI 助手。它会记住你，拔盘就走，连 3.5GB 的 Q1_0 量化小模型都能稳定干完智能体任务。

[![GitHub stars](https://img.shields.io/github/stars/swond-Mao/Tiffa?style=flat-square)](https://github.com/swond-Mao/Tiffa) [![License](https://img.shields.io/badge/license-MIT-blue.svg?style=flat-square)](https://github.com/swond-Mao/Tiffa/blob/master/LICENSE) [![Platform](https://img.shields.io/badge/platform-Windows-0078D6?style=flat-square)](https://github.com/swond-Mao/Tiffa) [![Portable](https://img.shields.io/badge/portable-U%E7%9B%98%E5%8D%B3%E7%94%A8-orange?style=flat-square)](https://github.com/swond-Mao/Tiffa) [![Zero C drive writes](https://img.shields.io/badge/zero_C_drive_writes-brightgreen?style=flat-square)](https://github.com/swond-Mao/Tiffa)

基于 [oh-my-pi v17.2.2](https://github.com/can1357/oh-my-pi) · MIT License · Windows

> 🔗 **基于开源框架 [oh my pi（OMP）](https://github.com/can1357/oh-my-pi) v17.2.2 深度定制**，非从零手搓。上游 ⭐：<https://github.com/can1357/oh-my-pi> · 官网 <https://omp.sh>

---

## 演示

<video src="demo/tiffa-demo.mp4" controls width="720"></video>

> 22 秒实机演示：启动仪式 → 多会话对话 → 操作电脑 → 记忆召回。视频文件位于 `docs/demo/`，可下载原片播放。

---

## 为什么是 Tiffa

你有没有过这种感觉——跟一个 AI 聊了三个月，它越来越懂你；然后换电脑、系统崩、重装，那个"它"不见了，你面对一个陌生人。**Tiffa 就是来解决这个问题的，而且不留下任何痕迹。**

| 能力 | 普通桌面助手 | OpenWebUI / Ollama | **Tiffa** |
|------|:---:|:---:|:---:|
| 记忆永续（换电脑 / 重装不丢） | ❌ | ⚠️ 会话级 | ✅ 分层记忆 + 语义召回 |
| 绝对隐私（不登录、不留云） | ❌ | ⚠️ 部分 | ✅ 全本地、拷走即走 |
| 弱模型也能稳定干活（Q1_0） | ❌ | ⚠️ | ✅ 七层约束兜底 |
| 一句话吐槽就改行为（免重启） | ❌ | ❌ | ✅ TTSR 即时生效 |
| 直接操控 Windows 桌面 | ❌ | ❌ | ✅ Computer Use |
| 整文件夹便携（U 盘即用） | ❌ | ❌ | ✅ |

---

## 适合谁

- **隐私敏感的你** — 公司电脑 / 共享电脑，不登录、不注册、不留痕，拔盘就走
- **多机党** — 家里一台、公司一台、U 盘随身，记忆跟着盘走
- **弱硬件玩家** — 3.5GB 的 Q1_0 小模型也能稳定干完智能体任务，老机器照样跑
- **AI 玩机党** — 7 套主题、自定义启动页动图、一句话吐槽改行为，可玩性拉满

---

## 核心特性

- **记忆永续** — 分层记忆（USER.md / 全局语义 / PROJECT.md / 项目语义）+ Mnemopi 本地 Embedding，换电脑记忆跟着走。
- **绝对隐私** — 不登录、不注册、不在服务器留任何记录；数据全在 U 盘。
- **弱模型可用** — 本地 / 云通吃，七层基础设施兜底，Q1_0 也能干活。
- **约束系统** — 三层约束（TTSR 13 条 / 行为约束 / tool_call 熔断），用代码管住模型。
- **目标模式** — 长任务先转写成带验收标准的方案、你点头才开工；目标逐轮钉进上下文不漂移，可自动续跑到完成。
- **操作电脑** — Computer Use v3：UIA 原子工具集 + 五级降级，直接操控 Windows 桌面。
- **思考档位切换** — 7 档思考深度，Ctrl+T 快捷键，旁路角色三层降级同步。
- **角色卡生成** — 内置面板一键塑造人格，本地模板兜底，旁路模型辅助定制。
- **PPTX 设计器** — 可视化编辑器 + 一键导出 PPTX/PDF + 真实图表渲染 + 换图面板。
- **模型健康检查** — 内置健康检测工具，一键验证模型可用性，自动诊断降级。
- **桌面前端** — Electron GUI（React + TypeScript），多工作区 / 多会话 / 模型选择器 / 双 Tab / Diff / 7 主题。
- **完全便携** — 一个文件夹就是全部，拷到 U 盘插上即用。

---

## 架构

![Tiffa 架构](architecture.png)

---

## 记忆永续

Tiffa 的记忆是一套分层的生命体，不是简单的文件存储：

```
L1  USER.md       — 你是谁，你的偏好
                    "我叫张三，讨厌废话开头，每次先说结论"
                    每轮必读，零延迟注入

L2  全局语义记忆  — 跨项目的使用历史
                    "用户有 RTX 5090，用 NSSM 管服务"
                    语义召回，永久积累，跨会话不丢失

L3  PROJECT.md    — 项目纲领
                    "本项目用 SQLite，缓存目录在 data/"
                    人工维护，切换项目时自动切换

L4  项目语义记忆  — 项目近期的进度和决策
                    "昨天修了 ComfyUI 管线，今天在调侧边栏"
                    语义召回，优先于全局记忆
```

关了重开，它还认得你。换个项目，它知道上下文。隔了一周，翻日志还能接上。

### Mnemopi 语义记忆

Embedding 模型跑在本地（`BAAI/bge-small-zh-v1.5`，512 维 ONNX，无需 API），记忆写入向量数据库，自动积累（每 2 轮写入）、自动召回（会话首条消息注入相关记忆）。

防膨胀：单次最多召回 10 条、注入上限 2000 token、旧记忆自动退化压缩——数据库可以无限长，上下文永远不爆。

长对话自动压缩：默认走 **snapcompact 视觉帧摘要**（有视觉模型时）；待压文本过大时（默认 > 2MB）自动降级到**旁路结构化总结**（便宜模型生成 9 段摘要），避免视觉帧撑爆上下文。弱模型也能低成本续接，工具细节语义级保留。

### 进度沉淀：日报 / 周报 / 月报

Tiffa 不只记"你说过什么"，还记"你做成了什么"。每次长对话压缩时，旁路模型把本次对话精炼成一行流水账，写入项目的 `.progress/log.md`；会话启动或跨天时自动聚合，向 `PROJECT.md` 沉淀出**日报 / 周报 / 月报**——你隔一周回来，翻开 PROJECT.md 就知道这段时间推进了什么、卡在哪、下一步是什么。

---

## 约束系统

Tiffa 的约束分三层，各司其职：

**第一重：TTSR 流式规则**（零 Context 成本）
- 写在 `.md` 文件里，模型输出时实时检测，违规立即拦截
- 13 条规则，涵盖废话开头、代码块格式、危险操作、工具调用废话等

**第二重：before_agent_start Hook**（语义约束）
- 读文件规范、任务计划方法、skill 铁律
- 系统 Prompt 前缀注入，行为 / 语义类约束专属

**第三重：tool_call Hook**（危险操作拦截）
- 危险路径拦截（System32 / Windows / Program Files）
- 配置文件自改拦截
- 静默工具调用检测（连续 3 次 → steer 提醒）

不满意模型的行为？直接一句话吐槽它：

```bat
/吐槽 你不应该把产物放在工作区根目录
```

中英文、文件摆放、啰嗦开头……什么毛病都能说。`/omfg` 同样能触发。模型会分析问题，生成一条 TTSR 规则文件，即时生效。不用改代码，不用重启。

---

## 用代码约束模型的行为

Tiffa 不靠"说得好听"的提示词逼模型就范，而是**用代码一层层约束模型的行为**——把容易犯错的环节拆成确定步骤，让模型一步步走完。弱模型因此能稳定完成任务；强模型则能按你的要求产出**确定性产物**，不跑偏、不擅自发挥。

**工匠模式**就是个例子：做交互网页、海报、演示时，Tiffa 走 craftman 多技能编排——先就主题 / 风格 / 是否生图问你确认，再按方案调用 `pptgen` / `comfyui` / `canvas-design`，最后合并输出。配套一道 **HTML 守卫**：任何直接写出、引用了视觉组件库（`shared-visual-components` / `data-theme=`）的 `.html`，若 craftman 没真正跑过，会被拦截并提示先走流程——"问了却不跑""手搓绕过"都堵死。整套流程是**代码兜底**的，不是靠模型自觉。

**不管弱模型还是强模型，都靠这套机制兜底：**

| 机制 | 效果 |
|------|------|
| TTSR 规则拦截 | 零 Context 成本约束，每轮省 ~500 token |
| 废话开头拦截 | 弱模型最高频犯蠢模式，直接砍掉 |
| XML 工具调用拦截 | 从"完全无法调工具"到能调 |
| 工具调用废话拦截 | 少说多做，省 token |
| 旁路结构化总结 | 长对话不断片，弱模型也能续上下文 |
| Loop Guard | 精确重复检测 + 自动重试 |

**旁路模型总结——我们引以为傲的一环。** 长对话压缩、进度沉淀都交给一个独立的旁路小模型，它有三个别人没有的好处：
- **第三方视角，不被上下文污染**——它不读主对话的逐步推理，只看到要总结的内容，结论更客观、不跟着模型一起跑偏；
- **便宜模型省钱**——用最便宜的小模型干总结这种体力活，主力模型只管干活；
- **不带 agent 约束、省 token**——旁路模型不加载主 agent 的系统提示与工具定义，上下文极轻，总结成本几乎可忽略。

**Q1_0 极限量化模型也能稳定干活。** 不是模型变强了，是基础设施补上了它的短板——强模型用这套底座，则更稳、更省、更可控。

---

## 操作电脑

不止写代码——Tiffa 能直接操控你的 Windows 桌面：

- **原子工具集**：`ui_inspect`（读控件树）/ `ui_act`（点击输入）/ `ui_screenshot`（截图）/ `desktop_input`（键鼠）/ `computer_use`（一体化）
- **五级降级**：UIA Pattern 直调 → 精确坐标点击 → SoM 编号标注 → 归一化坐标兜底 → OCR 文本识别
- **三阶段强制流程**：应用探测（不可跳过）→ 策略选择（有 CLI/API/COM 后台通道优先）→ 执行
- **默认 VLM**：豆包 `doubao-seed` 视觉模型，可在后台「MCP 模型」栏手配

> 老实说：它不算特别好用。复杂界面、动态弹窗、自绘控件仍有失手，现阶段更适合"点固定按钮、填固定表单"这类确定性任务。但它确实存在——本地助手里能直接操作桌面的，不多。

---

## 桌面前端

Electron GUI（React + TypeScript 渲染层），不是终端里的一行行文字：

- 项目侧栏 — 多工作区切换，归档不丢
- 会话标签 — 每个对话独立模型、独立记忆
- 模型选择器 — 一键切换本地 / 云模型
- 思考档位切换 — 7 档思考深度，Ctrl+T 快捷键，旁路角色三层降级同步
- 角色卡生成 — 内置面板一键塑造人格，本地模板兜底，旁路模型辅助定制
- 右侧栏双 Tab — 概要（Todo + 项目纲领）/ 文件（平面导航 + 抽屉预览）
- 文件抽屉 — HTML 渲染 / 代码高亮 / 图片居中 / Markdown 格式化
- Diff 视图 — 代码变更红绿分明
- 7 套主题 — 日夜模式一键切

### 目标模式（长任务）

给一个对话设**持久目标**。它解决的是长跑真正的风险：几十轮之后最初的验收标准被压缩没了，模型开始自己缩小定义、把「做了一半」报成「完成」。

1. 点输入框旁的靶心开关，写下你要完成的大任务；
2. 模型**先只做转写**——产出目标（写清「做完了是什么样子」）+ 验收标准 + 执行步骤。这一轮它只读代码，改文件、跑命令、创建目标全被拦住；
3. 方案出现在输入区上方的卡片里，**你可以直接改**，确认后点「开始执行」它才动手。

之后目标原文、验收标准与步骤会逐轮钉进上下文，模型不得改写或替换。收尾时它必须逐项对照当前仓库状态才能标记完成。

- **token 预算**（可选，开始时可设）：到顶时内核会把目标置为「预算耗尽」并要求模型停止开新活、留下交接。预算耗尽 ≠ 完成。
- **自动续跑**（可选）：一轮结束后自动发起下一轮，直到目标完成或撞上限。上限是硬护栏——轮数、时长、预算、被中止、你有排队消息，任一命中即停，并说明停下的原因；随时可暂停或接管。
- **定时开跑**：定时任务勾选「以目标模式运行」，到点先设定目标再执行。

⚠️ 能力边界：token 预算**只能在开始时设定，中途改不了**——内核的 goal 工具没有改预算的操作，改预算的接口也只被终端版调用。想放宽长跑上限，请调轮数与时长。同理，内核目标模式在终端交互外不提供「暂停目标」的操作，桌面端的「暂停」是停止续跑（目标与进度都保留）。

### 开局

启动不是干巴巴的转圈，是一场入局仪式：

```
  与万物对弈，伴时间同行      ← 主标题浮现
   夜色将尽，晨光初透       ← 唤醒引擎
   静水深流，暗涌潜行       ← 加载记忆
   行囊在肩，天地为卷       ← 整理上下文
   灯火已明，门扉待启       ← 准备就绪
```

**启动页还能放你自己的图。** 把 `startup-image.png/.jpg/.webp/.gif` 丢进 `data/` 文件夹，重启即生效（≤20MB，GIF / WebP 动图照放不误）。白底黑字图在暗色主题下**自动反色**成黑底白字，亮色主题保持原样——一张图通吃两套主题，从星空到你的作品，换肤只在一瞬间。
---

## 视觉设计与 PPT 生成

Tiffa 不只是对话助手——它还自带一套**可视化设计工具链**，帮你快速产出设计稿、PPT、海报：

- **PPTX 设计器** — 可视化编辑器 + 一键导出 PPTX/PDF + 真实图表渲染 + 换图面板，借鉴 HospAI 极简化 UI，支持页级模板目录和运行时渲染。
- **AI 生图** — 接入 ComfyUI 管线，支持多种生图模型，配合 prompt 模板和参数预设，直接生成高质量图片。
- **工匠模式** — 多技能编排引擎：先就主题、风格、是否生图与你确认，再按方案调用 PPTX 设计器 / ComfyUI / 画布设计，最后合并输出。配套 **HTML 守卫**：任何涉及视觉组件的 `.html` 若没真正跑过编排流程，会被拦截——"问了却不跑""手搓绕过"都堵死，整套流程是代码兜底的。

---

## 便携：拷走就走，不留痕迹

```
Tiffa/           ← 这一个文件夹就是全部
├── electron/    ← 桌面前端（主进程 TS 模块化 + React 渲染层）
│   ├── main.ts  ← 入口：配置 + IPC 路由 + 生命周期
│   └── modules/ ← 拆分模块（实例池 / 会话 / 项目 / 配置 / 窗口 / 工具）
├── npm-global/   ← Bun + 内核
├── plugins/     ← 扩展 + 技能
├── data/        ← 配置 + 记忆 + 会话 + 规则
└── workspace/   ← 你的项目
```

**拷到 U 盘，插上就用。**

- 不写注册表
- 不装全局包
- 不在 `%APPDATA%` / `%LOCALAPPDATA%` 留任何文件（userData 锁定到包内 `data/electron-userdata`，HOME / USERPROFILE 重定向到包内 `home/`）
- **npm / pip 缓存也指向包内** `.cache/`，安装与运行全程零 C 盘写入
- 工作区（workspace/）也在 U 盘里，不碰目标电脑的硬盘
- 不登录、不注册、不在服务器留任何记录

你在公司电脑用完，拔盘带走。回家插上，所有记忆、项目、配置原封不动。没有账号，没有云端，没有任何地方留存你的使用痕迹。

---

## 安装

```bat
install.bat        :: 一键安装（国内镜像，自动下载 Node / Bun / 内核 / Electron / Python / 离线 embedding 模型）
```

- 断点可重入：中断后重跑自动跳过已完成的步骤
- 安装过程中所有下载缓存都落在包内 `.cache/`，装完即可整包拷走
- 首次启动会引导你给 AI 起名字（默认 Tiffa）

---

## 工程化：CI / 测试 / 发布

- **CI 质量门禁**（GitHub Actions）：typecheck（主进程 TS + 渲染层 TS）→ 单测（main.test.js 21 项 + 主进程模块 vitest 74 项 + 渲染层 4 项 = 99 项）→ 构建（主进程 TS 编译 + 渲染层 Vite）→ 产物校验，红叉不收工
- **三层自动测试**：① 协议 E2E（真实内核 spawn 全链路）→ ② agent 自跑（让 Tiffa 给自己体检）→ ③ 浏览器 UI 验证
- **一键发布**：`cd electron && npm run release`——全检通过后自动打 tag、推三远端（gitee / github / gitcode）

---

## 启动

```bat
tiffa-desktop.exe      # 双击启动
tiffa-desktop.vbs      # 无控制台窗口
start-desktop.bat      # 带 --portable-root 参数
start-tiffa.bat       # TUI/WebUI/RPC 终端模式
```

或者开发模式：`cd electron && npm start`

---

## 截图

![启动仪式（默认星空图，可自定义）](screenshots/startup-default.webp)

> 📷 **实机截图待补充。** 建议放置：① 桌面 GUI 主界面 ② 记忆召回示意。
> 把图片存入 `docs/screenshots/`，然后取消下方注释即可显示：
>
> `<!-- ![桌面 GUI](docs/screenshots/desktop.png) -->`

---

## 技术彩蛋：七层改造

裸跑的模型就像一把没开刃的刀。Tiffa 的七层架构，是磨刀石：

```
  驱动指令 ─── 让模型知道"我是谁"
  权限系统 ─── 让模型知道"我能做什么"
  Hooks 链  ─── 在模型犯错时纠正
  规则系统 ─── 让模型知道"我不能做什么"
  记忆系统 ─── 让模型记住"你告诉过我什么"
  插件系统 ─── 让模型获得新能力
  技能系统 ─── 让模型知道"遇到这类事该怎么做"
```

七层合在一起，效果是：**Q1_0 极限量化模型也能稳定干活。**

---

## 致谢

Tiffa 站在 [oh my pi（OMP）](https://github.com/can1357/oh-my-pi) 的肩膀上。它是一套开源的 AI 编码智能体框架，Tiffa 的全部能力都生长在这套内核之上。如果你喜欢 Tiffa，请也去上游点个 Star ⭐，让更多人知道 OMP：<https://github.com/can1357/oh-my-pi> · 官网 <https://omp.sh>。

---

## 许可证

MIT。用就是了。