...
> 22 秒实机演示：启动仪式 → 多会话对话 → 操作电脑 → 记忆召回。视频文件位于 `docs/demo/`，可下载原片播放。

---

## 为什么是 Tiffa

你有没有过这种感觉——跟一个 AI 聊了三个月，它越来越懂你；然后换电脑、系统崩、重装，那个"它"不见了，你面对一个陌生人。**Tiffa 就是来解决这个问题的，而且不留下任何痕迹。**

| 能力 | 普通桌面助手 | OpenWebUI / Ollama | **Tiffa** |
|------|:---:|:---:|:---:|
| 记忆永续（换电脑 / 重装不丢） | ❌ | ⚠️ 会话级 | ✅ 分层记忆 + 语义召回 |
| 绝对隐私（不登录、不留云） | ❌ | ⚠️ 部分 | ✅ 全本地、拷走即走 |
| 弱模型也能稳定干活（Q1_0） | ❌ | ⚠️ | ✅ 七层约束兜底 |
...
- **记忆永续** — 分层记忆（USER.md / 全局语义 / PROJECT.md / 项目语义）+ Mnemopi 本地 Embedding，换电脑记忆跟着走。
...
中英文、文件摆放、啰嗦开头……什么毛病都能说。`/omfg` 同样能触发。模型会分析问题，生成一条 TTSR 规则文件，即时生效。不用改代码，不用重启。

---

## 用代码约束模型的行为

Tiffa 不靠"说得好听"的提示词逼模型就范，而是**用代码一层层约束模型的行为**——把容易犯错的环节拆成确定步骤，让模型一步步走完。弱模型因此能稳定完成任务；强模型则能按你的要求产出**确定性产物**，不跑偏、不擅自发挥。

**工匠模式**就是个例子：做交互网页、海报、演示时，Tiffa 走 craftman 多技能编排——先就主题 / 风格 / 是否生图问你确认，再按方案调用 `pptgen` / `comfyui` / `canvas-design`，最后合并输出。配套一道 **HTML 守卫**：任何直接写出、引用了视觉组件库（`shared-visual-components` / `data-theme=`）的 `.html`，若 craftman 没真正跑过，会被拦截并提示先走流程——"问了却不跑""手搓绕过"都堵死。整套流程是**代码兜底**的，不是靠模型自觉。
...
**旁路模型总结——我们引以为傲的一环。** 长对话压缩、进度沉淀都交给一个独立的旁路小模型，它有三个别人没有的好处：
- **第三方视角，不被上下文污染**——它不读主对话的逐步推理，只看到要总结的内容，结论更客观、不跟着模型一起跑偏；
- **便宜模型省钱**——用最便宜的小模型干总结这种体力活，主力模型只管干活；
- **不带 agent 约束、省 token**——旁路模型不加载主 agent 的系统提示与工具定义，上下文极轻，总结成本几乎可忽略。

**Q1_0 极限量化模型也能稳定干活。** 不是模型变强了，是基础设施补上了它的短板——强模型用这套底座，则更稳、更省、更可控。
