...
- [electron/build-icon.js](file://electron/build-icon.js)
...
## 更新摘要
**变更内容**   
- 新增图标生成系统：通过 electron/build-icon.js 自动创建多分辨率变体（16x16 到 512x512 PNG 文件）并生成包含嵌入分辨率的 Windows ICO 文件
- 修复 install.ps1 编码问题：添加 UTF-8 BOM 支持，解决 Windows 5.1 系统上中文字符解析错误
- 更新构建流程：集成图标生成到打包流程中，确保应用图标正确嵌入
...
**更新** 新增了完整的图标生成系统和 Windows 编码兼容性修复，确保跨平台部署的一致性和用户体验。
...
- 图标生成系统：build-icon.js 自动从 PNG 源图生成多分辨率变体和 Windows ICO 文件，并嵌入到可执行文件中。
- 配置中心：config.yml 定义模型角色与记忆后端；models.yml 定义各提供商模型端点、密钥与能力。

**更新** 新增图标生成系统，支持自动化的多分辨率图标处理和 Windows 可执行文件图标嵌入。
...
- [electron/build-icon.js:1-148](file://electron/build-icon.js#L1-L148)
...
**更新** 添加了 UTF-8 BOM 支持，解决 Windows 5.1 系统上中文字符解析错误的问题。
...
章节来源
- [install.ps1:1-217](file://install.ps1#L1-L217)

### 图标生成系统（build-icon.js）
**新增** 完整的图标生成和处理系统，支持以下功能：

- **多分辨率生成**：从单个 PNG 源图自动生成 16x16 到 512x512 的各种分辨率变体
- **ICO 文件创建**：生成符合 Windows 标准的 ICO 文件，包含多个分辨率嵌入
- **可执行文件图标嵌入**：自动将生成的图标嵌入到 tiffa-desktop.exe 中
- **Python 图像处理**：使用 PIL 库进行高质量的图像缩放和格式转换

```mermaid
flowchart TD
Source["tiffa-icon.png 源图"] --> Process["Python PIL 处理"]
Process --> Resize["生成多分辨率 PNG 变体"]
Resize --> ICO["创建 Windows ICO 文件"]
ICO --> Embed["嵌入到 tiffa-desktop.exe"]
Embed --> Output["输出图标文件和更新的 EXE"]
```

图表来源
- [electron/build-icon.js:33-83](file://electron/build-icon.js#L33-L83)
- [electron/build-icon.js:85-126](file://electron/build-icon.js#L85-L126)
...
**更新** 图标配置已更新为使用 assets/tiffa-icon.ico，支持多分辨率图标显示。
...
+build.win.icon : "assets/tiffa-icon.ico"
...
图表来源
- [electron/package.json:1-82](file://electron/package.json#L1-L82)
...
- CI/CD 建议
  ...
  - 步骤4：执行 electron/build-icon.js 生成图标并嵌入到可执行文件。
  - 步骤5：将产物上传至制品库，附带 models.yml.example 与 README。
  - 步骤6：发布时可选择跳过桌面快捷方式创建（SkipDesktop）。
- 企业环境部署
...
**更新** 新增图标生成步骤到 CI/CD 流程，确保分发包包含正确的应用图标。

章节来源
- [install.bat:1-11](file://install.bat#L1-L11)
- [install.ps1:1-217](file://install.ps1#L1-L217)
- [electron/package.json:6-10](file://electron/package.json#L6-L10)
- [electron/build-icon.js:128-148](file://electron/build-icon.js#L128-L148)
...
- 图标资源版本
  - 图标文件应纳入版本控制，确保图标一致性和品牌识别度。
  - 支持多分辨率图标，适应不同显示密度和平台要求。

**更新** 新增图标资源版本管理，确保应用图标在不同平台和分辨率下的一致性。
...
- Linux/macOS（概念性指导）
  ...
  - 图标文件使用 PNG 格式，支持不同分辨率需求。

**更新** 图标系统支持跨平台，Windows 使用 ICO 格式，其他平台使用 PNG 格式。
...
- 图标优化
  - 智能生成多分辨率图标，避免运行时缩放开销。
  - 使用高质量 LANCZOS 重采样算法，确保图标清晰度。

**更新** 新增图标优化策略，通过预生成多分辨率图标提升性能和视觉质量。

章节来源
- [electron/package.json:35-43](file://electron/package.json#L35-L43)
- [install.ps1:28-34](file://install.ps1#L28-L34)
- [start-tiffa.bat:15-33](file://start-tiffa.bat#L15-L33)
- [electron/build-icon.js:44](file://electron/build-icon.js#L44)
...
- 图标生成失败
  - 现象：图标生成脚本报错或图标未正确嵌入。
  - 处理：确认 Python PIL 库可用；检查源图标文件路径；验证生成的 ICO 文件格式。
- PowerShell 编码错误
  - 现象：install.ps1 在 Windows 5.1 上出现语法错误。
  - 处理：确保脚本文件具有 UTF-8 BOM；重新下载最新版本的 install.ps1。

**更新** 新增图标生成和 PowerShell 编码相关的故障排查指南。
...
- [start-tiffa.bat:49-60](file://start-tiffa.bat#L49-L60)
- [data/agent/models.yml:1-242](file://data/agent/models.yml#L1-L242)
- [electron/build-icon.js:129-136](file://electron/build-icon.js#L129-L136)
...
**更新** 新增的图标生成系统进一步提升了应用的专业性和用户体验，确保在不同平台和分辨率下都能显示清晰的应用图标。UTF-8 BOM 支持解决了 Windows 5.1 系统的兼容性问题，使部署更加稳定和可靠。