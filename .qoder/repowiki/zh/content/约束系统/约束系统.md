...
## 更新摘要
**变更内容**   
- 移除了 XML 工具翻译器功能（plugins/xml-tool-translator.ts 已删除）
- 更新了内核原生工具调用机制的说明
- 强化了 tool_call Hook 的安全拦截机制
- 完善了 before_agent_start Hook 的语义约束实现
...
**重要更新**：XML 工具翻译器功能已被完全移除，系统现在使用内核原生工具调用机制，提供更高效、更安全的工具执行环境。

## 项目结构
与约束系统直接相关的代码与规则位于以下位置：
- 主进程与事件转发：electron/main.js
- 扩展插件（工具调用拦截）：plugins/computer-use-extension.ts
...
subgraph "扩展插件层"
E["computer-use-extension.ts<br/>工具调用拦截与语义注入"]
end
subgraph "Agent 配置"
...
R4["tool-call-commentary.md"]
R5["cwd-file-placement.md"]
end
M --> E
M --> C
M --> R1
...
- [plugins/computer-use-extension.ts:24-96](file://plugins/computer-use-extension.ts#L24-L96)
...
- before_agent_start Hook（语义约束）
  - 通过扩展插件在 Agent 启动前注入全局策略与行为边界。
  - computer-use-extension.ts 实现了电脑控制工具的语义约束注入。
- tool_call Hook（危险操作拦截）
  - 对工具调用参数进行匹配与拦截，防止危险操作（如强制推送、全量暂存、硬编码密钥等）。
  - 支持 bash/shell 命令的危险操作检测和安全限制。

**更新**：移除了 XML 工具翻译器，现在直接使用内核原生工具调用机制，提高了安全性和性能。
...
- 作用时机：Agent 启动前，通过扩展插件注入全局策略与行为边界。
- 实现方式：computer-use-extension.ts 监听 before_agent_start 事件，注入系统提示词。
- 配置入口：data/agent/config.yml 中的 tools.approvalMode 等字段影响工具调用策略。
- 与规则协同：结合 rules/*.md 的语义约束，确保 Agent 从启动阶段就遵循规范。

**更新**：现在通过扩展插件机制实现更灵活的语义约束注入，替代了原有的 XML 工具翻译器。
...
- 实现方式：computer-use-extension.ts 监听 tool_call 事件，对 bash/shell 命令进行安全检查。
- 匹配方式：通过 rule 的 scope 指定 tool:<name>(<glob>)，并结合 condition 精准匹配参数。
- 处置策略：根据 interruptMode 决定是否中断生成，并在 Markdown 正文给出纠正指导。
- 安全增强：检测危险命令（格式化、关机、注册表修改、系统进程终止等）。

**重要更新**：移除了 XML 工具翻译器后，工具调用现在直接通过内核原生机制执行，安全性更高，性能更好。
...
**更新**：随着 XML 工具翻译器的移除，no-xml-toolcall 规则现在主要用于防止模型尝试使用 XML 格式调用工具，确保使用标准的函数调用格式。
...
- **性能优化**：移除 XML 工具翻译器后，工具调用路径更短，减少了不必要的转换开销。
...
- **新增排查项**：扩展插件未加载
  - 检查 computer-use-extension.ts 是否正确加载。
  - 确认 before_agent_start 和 tool_call 事件监听器是否正常注册。
...
## 结论
Tiffa 的三重约束系统通过"流式规则 + 启动前语义约束 + 工具调用拦截"形成闭环，既保证低延迟与高可控性，又提供灵活的扩展能力。/omfg 命令使团队能够快速沉淀经验为规则，持续完善约束体系。

**重要更新**：随着 XML 工具翻译器的移除，系统现在使用更高效、更安全的内核原生工具调用机制。建议在实践中遵循"精确匹配、最小范围、清晰纠正"的原则，配合合理的配置与调试手段，构建稳定可靠的 Agent 行为边界。
...
- 安全策略配置
  ...
  - 利用扩展插件实现更复杂的安全检查和业务逻辑。
- **新增**：扩展开发指南
  - 通过 computer-use-extension.ts 了解如何编写自定义扩展插件。
  - 使用 before_agent_start 钩子注入系统提示词和行为约束。
  - 使用 tool_call 钩子实现工具调用的安全拦截和验证。