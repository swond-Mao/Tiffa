...
## 更新摘要
**变更内容**   
- 将扁平项目列表升级为层次化会话树界面，支持懒加载和展开/折叠
- 新增激活指示器显示已打开的会话状态
- 实现双向标签同步机制，保持会话树与顶部标签页的一致性
- 添加侧边栏宽度拖拽调整功能并持久化存储
- 优化项目选择、会话切换和右键菜单操作体验
...
R_CSS["styles.css<br/>侧边栏样式/拖拽手柄"]
end
subgraph "预加载桥接"
...
- [electron/renderer/styles.css:104-1335](file://electron/renderer/styles.css#L104-L1335)
...
## 核心组件
- 项目会话树界面：渲染层渲染、右键菜单、点击选择、懒加载展开
- 会话标签与历史面板：新建对话、切换、关闭标签、归档/删除会话
- 实例管理器：TiffaInstance 生命周期、LRU 淘汰、自动重启、事件转发
- 持久化：projects.json、sessions 目录、localStorage 打开标签记忆、侧边栏宽度
...
- [electron/renderer/styles.css:5370-5394](file://electron/renderer/styles.css#L5370-L5394)
...
### 层次化项目会话树界面
- 懒加载：项目展开时按需加载会话列表，避免初始加载大量数据
- 展开/折叠：点击箭头图标切换项目展开状态，支持嵌套会话组织
- 激活指示器：已打开的会话在树中显示高亮圆点，当前活跃会话整行高亮
- 双向同步：会话树与顶部标签页状态实时同步，确保一致性
...
Render --> ExpandCheck{"是否展开项目?"}
ExpandCheck -- "否" --> End(["完成"])
ExpandCheck -- "是" --> LazyLoad["懒加载会话列表"]
LazyLoad --> RenderTree["渲染会话树"]
RenderTree --> ActiveCheck{"是否有活跃会话?"}
ActiveCheck -- "是" --> ShowIndicator["显示激活指示器"]
ActiveCheck -- "否" --> End
ShowIndicator --> End
```

**图表来源**
- [electron/renderer/app.js:345-358](file://electron/renderer/app.js#L345-L358)
- [electron/renderer/app.js:1766-1827](file://electron/renderer/app.js#L1766-L1827)
...
- 拖拽手柄：右侧边缘提供可拖拽的调整手柄
- 实时调整：鼠标拖动时实时更新侧边栏宽度
- 边界限制：最小宽度200px，最大宽度为窗口宽度的60%
- 持久化存储：调整后的宽度保存到localStorage，下次启动恢复
...
+toggleExpandProject(dirName, forceOpen)
}
class SessionTree {
+renderSessionTreeInto(container, dirName, sessions)
+openSessionFromTree(sessionPath)
+refreshSessionTreeWithRetry(dirName, expectPath)
}
class ArchivedPanel {
+renderArchived()
...
ProjectPanel --> SessionTree : "渲染会话树"
...
- 持久化：
  ...
  - localStorage：每个项目的打开标签集合与活跃标签、侧边栏宽度
...
Restore --> LoadSidebarWidth["恢复侧边栏宽度"]
LoadSidebarWidth --> End(["完成"])
```
...
- 侧边栏拖拽失效：检查拖拽手柄元素是否存在，确认CSS样式正常加载
...
## 结论
项目侧栏以清晰的三层架构（渲染/预加载/主进程）实现了稳定的项目管理与多会话体验。通过懒启动、LRU 淘汰、事件驱动与本地持久化，兼顾了性能与可用性。新的层次化会话树界面提供了更好的组织结构，侧边栏拖拽调整增强了用户体验。开发者可基于现有 IPC 接口扩展更多项目管理能力（如批量导入、模板化项目）。
...
- 项目与会话
  ...
  - listSessions(projectDirName): Promise<Session[]>
  - switchSession(sessionPath): Promise<void>
  - newSession(): Promise<void>
  - loadSessionHistory(sessionPath): Promise<{messages:[], error?}>
  - archiveProject(dirName, cwd): Promise<{success|error}>
  - deleteProject(dirName, cwd): Promise<{success|error}>
...
- 调整侧边栏宽度
  - 监听拖拽事件，实时更新sidebar.style.width，并在mouseup时保存到localStorage
