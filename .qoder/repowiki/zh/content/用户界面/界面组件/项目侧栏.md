...
## 更新摘要
**变更内容**   
- 左侧边栏完全重新设计，采用“项目→会话”层级树结构
- 支持通过拖拽操作调整宽度，并使用 localStorage 持久化
- 增强了会话管理功能，包括跨项目标签页管理和懒加载机制
...
**章节来源**
- [electron/renderer/app.js:853-942](file://electron/renderer/app.js#L853-942)
- [electron/main.js:325-570](file://electron/main.js#L325-570)
- [electron/main.js:1165-1253](file://electron/main.js#L1165-1253)
...
**图表来源**
- [electron/renderer/app.js:853-871](file://electron/renderer/app.js#L853-871)
- [electron/preload.js:62-100](file://electron/preload.js#L62-100)
- [electron/main.js:809-823](file://electron/main.js#L809-823)
- [electron/main.js:325-451](file://electron/main.js#L325-451)
...
**图表来源**
- [electron/renderer/app.js:802-851](file://electron/renderer/app.js#L802-851)
- [electron/main.js:809-823](file://electron/main.js#L809-823)

**章节来源**
- [electron/renderer/app.js:802-851](file://electron/renderer/app.js#L802-851)
- [electron/renderer/app.js:1132-1299](file://electron/renderer/app.js#L1132-1299)
- [electron/main.js:1099-1124](file://electron/main.js#L1099-1124)
...
**章节来源**
- [electron/renderer/app.js:853-871](file://electron/renderer/app.js#L853-871)
- [electron/renderer/app.js:1301-1312](file://electron/renderer/app.js#L1301-1312)
...
**图表来源**
- [electron/renderer/app.js:484-546](file://electron/renderer/app.js#L484-546)
- [electron/renderer/app.js:648-701](file://electron/renderer/app.js#L648-701)
- [electron/main.js:144-173](file://electron/main.js#L144-173)
...
**图表来源**
- [electron/renderer/app.js:873-942](file://electron/renderer/app.js#L873-942)
- [electron/renderer/app.js:944-1130](file://electron/renderer/app.js#L944-1130)
- [electron/renderer/app.js:1319-1463](file://electron/renderer/app.js#L1319-1463)
...
**图表来源**
- [electron/main.js:1666-1702](file://electron/main.js#L1666-1702)
- [electron/renderer/app.js:1556-1583](file://electron/renderer/app.js#L1556-1583)
- [electron/renderer/app.js:1136-1147](file://electron/renderer/app.js#L1136-1147)
...
**章节来源**
- [electron/main.js:826-856](file://electron/main.js#L826-856)
- [electron/preload.js:47-51](file://electron/preload.js#L47-51)

### 新增：项目侧栏宽度调节功能
- 拖拽调节：通过 sidebarResizeHandle 元素实现拖拽调整宽度
- 持久化存储：使用 localStorage 保存 'tiffa:projectPanelWidth' 键值
- 边界控制：最小宽度160px，最大宽度420px，防止过度缩小或扩大
- 视觉反馈：拖拽过程中添加 'dragging' 类名提供视觉反馈

**章节来源**
- [electron/renderer/app.js:1290-1326](file://electron/renderer/app.js#L1290-1326)

### 新增：项目→会话层级树结构
- 层级展示：项目作为父节点，会话作为子节点显示
- 懒加载：点击展开箭头时才加载会话列表，提升性能
- 状态同步：活跃会话和打开标签在树中都有相应标记
- 交叉导航：支持跨项目标签页的无缝切换

**章节来源**
- [electron/renderer/app.js:240-297](file://electron/renderer/app.js#L240-297)
- [electron/renderer/app.js:1407-1510](file://electron/renderer/app.js#L1407-1510)
...
**图表来源**
- [electron/preload.js:24-104](file://electron/preload.js#L24-104)
- [electron/main.js:1-29](file://electron/main.js#L1-29)
...
- **新增优化**：项目侧栏宽度调节使用 requestAnimationFrame 确保流畅性
...
- **新增问题**：侧栏宽度异常：检查 localStorage 中 'tiffa:projectPanelWidth' 键值是否有效

**章节来源**
- [electron/main.js:1165-1253](file://electron/main.js#L1165-1253)
- [electron/main.js:826-856](file://electron/main.js#L826-856)
- [electron/renderer/app.js:703-730](file://electron/renderer/app.js#L703-730)

## 结论
项目侧栏以清晰的三层架构（渲染/预加载/主进程）实现了稳定的项目管理与多会话体验。通过懒启动、LRU 淘汰、事件驱动与本地持久化，兼顾了性能与可用性。开发者可基于现有 IPC 接口扩展更多项目管理能力（如批量导入、模板化项目）。

**更新后的特性**：
- 全新的项目→会话层级树结构，提供更好的导航体验
- 可调节的侧栏宽度，支持个性化布局需求
- 增强的会话管理能力，支持跨项目标签页管理
...
- **新增示例**：调节侧栏宽度
  - 监听 sidebarResizeHandle 的 mousedown 事件，实现拖拽逻辑
  - 使用 localStorage.setItem('tiffa:projectPanelWidth', width) 保存宽度设置
