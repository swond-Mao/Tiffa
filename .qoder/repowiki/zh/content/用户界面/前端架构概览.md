...
## 更新摘要
**变更内容**   
- 新增流式渲染节流机制，防止大数据加载时UI冻结
- 优化历史加载机制，从双遍JSON解析改进为单遍解析器同时处理tool_execution_start和message事件
- 增强minimap性能优化，使用Canvas绘制密度滚动条
- 实现懒加载历史消息，首屏只渲染最后40条消息
- 添加启动预加载机制，利用欢迎页空闲时间预载数据
...
MINIMAP["Minimap<br/>Canvas密度滚动条"]
FOLLOW["FollowScroll<br/>流式跟随控制"]
end
subgraph "外部运行时"
BUN["Bun 运行时"]
...
- [electron/renderer/app.js:778-836](file://electron/renderer/app.js#L778-L836)
...
- 主进程（main.js）
  ...
  - **新增**：优化的历史加载机制，单遍解析JSONL文件提升性能
  - 文件系统、Shell、模型配置等能力封装
- 预加载脚本（preload.js）
  - 使用 contextBridge 暴露受控 API
  - 统一封装 ipcRenderer.invoke/on，屏蔽底层细节
  - **新增**：markedNoHighlight 优化，禁用高亮提升长会话性能
- 渲染进程（app.js + index.html）
  - 全局 state 驱动 UI
  - 事件总线 handleEvent 分发
  - **新增**：流式渲染节流（requestAnimationFrame），防止高频更新导致UI冻结
  - **新增**：Minimap Canvas密度滚动条，批量绘制降低DOM操作开销
  - **新增**：懒加载历史消息，首屏只渲染最后40条
  - **新增**：启动预加载机制，利用欢迎页空闲时间预载数据
  - 卡住检测、首次响应超时、崩溃恢复提示
...
- [electron/main.js:2558-2671](file://electron/main.js#L2558-L2671)
...
- [electron/renderer/app.js:778-836](file://electron/renderer/app.js#L778-L836)
- [electron/renderer/app.js:3030-3169](file://electron/renderer/app.js#L3030-L3169)
- [electron/renderer/app.js:484-646](file://electron/renderer/app.js#L484-L646)
- [electron/renderer/themes.js:1-200](file://electron/renderer/themes.js#L1-L200)
- [electron/renderer/styles.css:1-84](file://electron/renderer/styles.css#L1-L84)

## 架构总览
下图展示从用户输入到内核响应的完整调用链，以及事件回传渲染进程的路径，包含新增的性能优化环节。
...
R->>R : requestAnimationFrame节流
R->>R : Minimap重绘调度
Note over R,S : 历史加载优化：单遍解析JSONL
M->>M : sessions : loadHistory (优化版)
M-->>R : {messages, total} (已合并tool_meta)
```
...
- 预加载层（preload.js）
  ...
  - **新增**：markedNoHighlight 优化，禁用代码高亮提升长会话性能
- 主进程层（main.js）
  - setupIpc() 注册所有 ipcMain.handle
  - 所有 Tiffa 命令通过 tiffaManager.resolve(cwd, sessionId) 路由到当前活跃实例或指定会话实例
  - **新增**：sessions:loadHistory 优化，单遍解析JSONL文件，同时收集tool_execution_start参数和处理message事件
...
- 全局 state
  ...
  - **新增**：historyCache、projectSessions 用于启动预加载缓存
  - **新增**：historyPending 用于懒加载队列管理
- 事件处理
  - handleEvent 集中分发，按类型更新状态与 DOM
  - **新增**：流式渲染节流，使用 requestAnimationFrame 合并高频更新
  - **新增**：Minimap 密度滚动条，Canvas绘制减少DOM操作
...
- UI 初始化
  ...
  - **新增**：preloadDuringWelcome() 在欢迎页期间预加载历史数据和会话列表
...
SetReady --> Preload["preloadDuringWelcome()<br/>预加载历史数据"]
Preload --> ShowWelcome{"是否有历史消息?"}
...
- [electron/renderer/app.js:902-936](file://electron/renderer/app.js#L902-L936)
- [electron/renderer/app.js:484-646](file://electron/renderer/app.js#L484-L646)
...
APP --> MINIMAP["Minimap Canvas"]
APP --> FOLLOW["FollowScroll"]
```
...
### 流式渲染节流
- **问题**：流式生成每个token都会触发DOM更新，高频更新导致UI卡顿
- **解决方案**：使用 `requestAnimationFrame` 节流，将多次更新合并到每帧一次
- **实现**：`followScroll.schedule()` 方法，通过 `pending` 标志防止重复调度
- **效果**：大幅减少重排重绘次数，提升流畅度

### 历史加载优化
- **问题**：大文件历史加载需要两次JSON.parse，性能开销大
- **解决方案**：单遍解析JSONL文件，同时收集 `tool_execution_start` 参数和处理 `message` 事件
- **实现**：`sessions:loadHistory` 中的 `toolMeta` Map 存储工具元数据，后续 `toolResult` 直接复用
- **效果**：减少50%的JSON.parse开销，显著提升大会话加载速度

### Minimap密度滚动条
- **技术**：纯Canvas 2D绘制，零依赖实现密度滚动条
- **性能优化**：
  - `redrawPending` 标志防止一帧内多次重绘
  - `requestAnimationFrame` + `setTimeout(100ms)` 双通道触发
  - DPR适配确保高分屏清晰度
  - 只遍历 `.message` 元素，减少DOM查询开销
- **功能**：实时反映消息密度，支持拖拽跳转，自动跟随主题切换

### 懒加载历史消息
- **策略**：首屏只渲染最后40条消息（`HISTORY_LAZY_BATCH = 40`），更早的消息存入待渲染队列
- **实现**：`buildHistoryFragment()` 构建DocumentFragment，`loadEarlierBatch()` 按需加载
- **用户体验**：顶部显示"加载更早消息"按钮，点击后增量加载，保持视口位置稳定

### 启动预加载机制
- **时机**：利用欢迎页启动期间的空闲时间
- **内容**：
  - 预加载已打开tab的历史会话（排除当前活跃和__new__）
  - 预加载所有项目的会话列表
- **节流**：每次间隔100ms，避免IPC洪水
- **效果**：进入会话时无需等待，实现秒开体验

### 网络与IO优化
- 首次响应超时与卡住检测提升用户体验
- 文件读取/写入通过主进程统一封装，避免渲染进程直接访问 fs
- **新增**：大文件（>20MB）只读取最后10MB，跳过不完整的第一行
...
- 常见问题定位
  ...
  - **新增**：历史加载失败：检查 epoch 防竞态机制，确认预加载缓存是否命中
  - **新增**：UI卡顿：检查 requestAnimationFrame 节流是否正常工作，Minimap重绘是否频繁
- 调试建议
  ...
  - **新增**：性能监控：关注控制台日志中的 `[loadAndRenderHistory]` 和 `[preload]` 相关输出
...
- [electron/renderer/app.js:3074-3151](file://electron/renderer/app.js#L3074-L3151)

## 结论
Tiffa 前端采用清晰的三层架构：主进程负责实例与 IPC、预加载层提供安全 API、渲染进程专注状态与交互。通过 JSONL 协议与 Bun 运行时高效对接 Tiffa 内核，结合 LRU 与自动重启策略保障稳定性；渲染进程以全局 state 驱动 UI，具备完善的错误检测与调试能力。

**最新优化亮点**：
- **流式渲染节流**：通过 requestAnimationFrame 合并高频更新，防止UI冻结
- **历史加载优化**：单遍解析JSONL文件，同时处理tool_execution_start和message事件，性能提升显著
- **Minimap密度滚动条**：纯Canvas实现，零依赖高性能滚动导航
- **智能懒加载**：首屏只渲染必要内容，大幅提升大会话加载速度
- **启动预加载**：利用空闲时间预载数据，实现无缝切换体验

该设计便于扩展新能力（如新增 IPC 通道、会话功能、主题等），同时保持低耦合与高性能。
