...
- [mcp.json](file://data/agent/mcp.json)
- [main.js](file://electron/main.js)
- [test_memory_system.ts](file://test_memory_system.ts)
- [memory-manager SKILL.md](file://skills/memory-manager/SKILL.md)
- [computer_use_mcp.py](file://skills/computer-use/computer_use_mcp.py)
</cite>

## 更新摘要
**已进行的更改**
- 新增每日记忆文件系统的详细说明，包含 `.workbuddy/memory/` 目录结构和组织方式
- 更新 Computer Use MCP 服务器集成配置和使用方法
- 完善三层记忆架构（USER.md、MEMORY.md、daily-log）的实现细节
- 新增记忆管理器 MCP 工具的使用指南
- 更新配置选项和性能优化建议
...
- 配置与记忆
  ...
  - data/agent/mcp.json：MCP 服务器配置（含 Computer Use）
  - data/memory/constraints-inject.md：行为约束（before_agent_start 注入）
  - .workbuddy/memory/YYYY-MM-DD.md：每日记忆文件
...
C --> F["MCP 工具<br/>memory-manager 工具集"]
F --> G["每日记忆文件<br/>.workbuddy/memory/"]
A --> H["MCP 服务器<br/>computer-use mcp.json"]
A --> I["模型配置<br/>data/agent/models.yml"]
A --> J["会话与项目数据<br/>sessions/projects.json"]
```

**图表来源**
- [main.js:29-31](file://electron/main.js#L29-L31)
- [config.yml:13-27](file://data/agent/config.yml#L13-L27)
- [mcp.json:1-18](file://data/agent/mcp.json#L1-L18)
...
- **新增** 每日记忆系统
  - daily-log/YYYY-MM-DD.md：按日期组织的结构化日志
  - 通过 memory-manager MCP 工具进行读写操作
  - 支持时间衰减和自动清理
- 注入与触发
...
Agent->>MCP : memory_save/memory_search/memory_get
MCP->>Daily : 读写 YYYY-MM-DD.md 文件
Agent-->>UI : 响应结果
```
...
### **新增** 每日记忆文件系统
- 存储位置：`.workbuddy/memory/YYYY-MM-DD.md`
- 文件格式：Markdown 格式，包含日期标题和结构化内容
- 内容类型：
  - 日常对话内容、工作进展、临时信息
  - 技术问题和解决方案记录
  - 项目进度和里程碑
  - 用户反馈和改进建议
- 访问方式：通过 memory-manager MCP 工具的 `memory_get(file="today")` 获取当日日志
- 生命周期：按时间衰减，支持自动清理过期文件

```mermaid
flowchart TD
Start(["开始新的一天"]) --> Create["创建 YYYY-MM-DD.md 文件"]
Create --> Log["记录日常活动"]
Log --> Categorize{"内容分类"}
Categorize --> |用户偏好| UserMD["写入 USER.md"]
Categorize --> |核心事实| MemoryMD["写入 MEMORY.md"]
Categorize --> |日常记录| DailyLog["写入 daily-log"]
UserMD --> End(["完成"])
MemoryMD --> End
DailyLog --> End
```
...
**图表来源**
- [memory-manager SKILL.md:1-56](file://skills/memory-manager/SKILL.md#L1-L56)
...
### **新增** Memory Manager MCP 工具
- 工具名称：memory-manager
- 功能描述：长期记忆管理，支持跨会话记忆保存、搜索和读取
- 三个核心工具：
  - `memory_save(content, category)`：保存记忆，支持 user/core/daily 三类
  - `memory_search(query, max_results)`：搜索记忆，支持正则表达式
  - `memory_get(file)`：读取记忆文件，支持特殊文件名如 "today"、"USER.md"、"MEMORY.md"
- 存储位置：`$env:PORTABLE_ROOT/data/memory/` 目录
- 使用场景：
  - 用户说"记住 XXX"时自动整理并保存
  - 用户查询历史记录时进行搜索
  - 需要回顾特定信息时读取对应文件
...
### **新增** Computer Use MCP 服务器集成
- 配置文件：`data/agent/mcp.json`
- 服务器类型：stdio 标准输入输出
- Python 脚本：`skills/computer-use/computer_use_mcp.py`
- 功能特性：
  - 8 个 MCP 工具：ui_inspect、ui_act、ui_screenshot、ui_find_text、ui_click_text、ui_ocr、desktop_input、computer_use
  - 三阶段强制流程：应用探测 → 策略选择 → 执行
  - 五级降级机制：L1 UIA Pattern → L2 精确坐标 → L3 SoM 标注 → L4 归一化坐标 → L5 OCR 识别
- 开关控制：
  - 默认关闭，避免启动时加载重型依赖
  - 通过 `data/agent/computer-use-enabled` 文件控制
  - 支持运行时切换和 IPC 接口

**章节来源**
- [mcp.json:1-18](file://data/agent/mcp.json#L1-L18)
- [computer_use_mcp.py:1-1258](file://skills/computer-use/computer_use_mcp.py#L1-L1258)
...
- 组件耦合
  ...
  - Mnemopi 负责向量存储与检索
  - **新增** MCP 工具负责 memory-manager 和 computer-use 功能
- 外部依赖
  - models.yml：多 Provider 模型配置（本地/云端）
  - Bun/Node/Python：运行时环境
  - **新增** Python 依赖：pywinauto、comtypes、mss、Pillow、pyautogui 等
- 潜在循环依赖
...
- **新增** MCP 服务器优化
  - Computer Use MCP 默认关闭，避免启动时加载重型 Python 依赖
  - 懒加载机制：uia_core 仅在首次工具调用时导入
...
- **新增** 每日记忆文件问题
  - 现象：无法读取或写入 daily-log 文件
  - 解决：检查 `$env:PORTABLE_ROOT/data/memory/` 目录权限和路径正确性
- **新增** Computer Use MCP 问题
  - 现象：MCP 服务器启动失败或工具调用异常
  - 解决：检查 `data/agent/mcp.json` 配置、Python 环境依赖、`computer-use-enabled` 文件状态
- **新增** 双层召回问题
...
## 结论
Tiffa 的四重记忆系统通过"文件记忆 + 语义记忆 + 每日日志"的分层设计，实现了跨会话、跨项目的持久化与智能检索。**最新的每日记忆文件和 Computer Use MCP 服务器集成进一步增强了系统的功能性和实用性**。Mnemopi v6.1 的引入使系统在弱模型环境下仍能提供稳定的上下文增强与断片恢复能力。配合 Electron 主进程的预热、路径迁移与 IPC 管理，整体架构具备高可用性与可移植性。
...
- 快速参考
  ...
  - **新增** Memory Manager 工具使用方法见 skills/memory-manager/SKILL.md
  - **新增** Computer Use MCP 配置见 data/agent/mcp.json
- **新增** 配置验证
