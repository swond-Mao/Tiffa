...
- 关键能力
  ...
  - 召回策略：autoRecall=false（关闭自动召回），manual recall 可控；autoRetain=true（每次 agent_end 自动 retain）
  - 保留频率：retainEveryNTurns=2（每 2 轮 retain 一次）
  - 注入上限：injectionTokenLimit=2000（防止撑爆上下文）
  - scoping：global（所有项目共享同一库，metadata.cwd 标记来源）
...
- 跨项目
  ...
  - Mnemopi scoping=global，通过 metadata.cwd 区分来源，实现跨项目语义检索
...
## 结论
Tiffa 的四重记忆系统通过“文件记忆 + 语义记忆”的分层设计，实现了跨会话、跨项目的持久化与智能检索。Mnemopi v6.1 的引入使系统在弱模型环境下仍能提供稳定的上下文增强与断片恢复能力。配合 Electron 主进程的预热、路径迁移与 IPC 管理，整体架构具备高可用性与可移植性。
