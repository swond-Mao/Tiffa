...
## 更新摘要
**所做更改**   
- 统一了所有技能的 SKILL.md 文档格式，采用标准化的 YAML 头部结构
- 移除了硬编码路径和 OpenCode 引用，替换为可移植的 PORTABLE_ROOT 变量
- 更新了平台品牌标识，从 'opencode' 改为 'Tiffa'
- 增强了跨会话历史检索和记忆管理功能的文档说明
...
**最新更新**：所有技能已标准化 SKILL.md 格式，使用 PORTABLE_ROOT 环境变量实现跨平台兼容性，平台品牌统一为 Tiffa。
...
- **更新**：现在使用 PORTABLE_ROOT 变量替代硬编码路径，支持跨平台部署。
...
- **更新**：完全移除 OpenCode 环境依赖，使用 PORTABLE_ROOT 变量实现 Tiffa 便携包兼容。
...
- **更新**：使用 PORTABLE_ROOT 变量替代硬编码路径，支持跨平台部署。
...
**图示来源** 
- [skills/pptgen/SKILL.md:1-50](file://skills/pptgen/SKILL.md#L1-50)
...
- **更新**：记忆文件存储在 `$env:PORTABLE_ROOT/data/memory/` 目录，支持跨平台部署。

**章节来源**
- [skills/memory-manager/SKILL.md:1-90](file://skills/memory-manager/SKILL.md#L1-L90)

### 历史检索：history-query（跨会话历史检索）
- 功能特性：跨会话历史检索，使用 Python sqlite3 直读 Tiffa 数据库。
- 数据库位置：`$env:PORTABLE_ROOT/data/agent/history.db`
- 使用方法：编写临时 Python 脚本执行查询，支持关键词搜索和完整会话查看。
- **更新**：完全使用 PORTABLE_ROOT 变量，移除硬编码路径。

**章节来源**
- [skills/history-query/SKILL.md:1-72](file://skills/history-query/SKILL.md#L1-L72)
...
- 系统与运行时：
  ...
  - 远程服务（ComfyUI 服务器地址）
- **更新**：所有路径现在使用 PORTABLE_ROOT 环境变量，支持跨平台部署。
...
Memory["memory-manager"] --> PortRoot["PORTABLE_ROOT/data/memory/"]
History["history-query"] --> DB["PORTABLE_ROOT/data/agent/history.db"]
```
...
- **更新**：PORTABLE_ROOT 变量的使用提升了跨平台兼容性，减少了路径相关的性能问题。
...
- **新增**：PORTABLE_ROOT 路径问题：
  - 症状：文件路径错误或权限问题；检查 PORTABLE_ROOT 环境变量是否正确设置。
  - 参考：skills/history-query/SKILL.md, skills/memory-manager/SKILL.md
...
## 结论
Tiffa 技能系统以 SKILL.md 为核心元数据载体，结合统一的 skill 工具与清晰的依赖管理，实现了从文档处理、图像生成到演示文稿与图表绘制的完整能力闭环。通过分层记忆、三重约束与 Hook 机制，系统在弱模型环境下仍保持稳定执行。最新标准化改进包括统一的 SKILL.md 格式、PORTABLE_ROOT 变量支持和 Tiffa 品牌标识，提升了跨平台兼容性和可维护性。建议在实际使用中遵循"先问后做"的行为约束、充分利用降级策略与质量门禁，并在自定义技能开发时保持元数据一致性与依赖最小化。
...
- 参数与环境变量：
  ...
  - 便携包：PORTABLE_ROOT（必需）
  - 其他：PI_CODING_AGENT_DIR、HOME/USERPROFILE
...
- **更新**：使用 PORTABLE_ROOT 变量替代硬编码路径，确保跨平台兼容性。
