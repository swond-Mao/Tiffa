...
## 更新摘要
**所做更改**
- 增强了 ComfyUI 图像生成管道，实现了标准化的 API 端点
- 改进了输出目录解析机制，支持多级优先级配置
- 在所有工作流中添加了批量处理支持
- 优化了 HTTP 请求处理和错误恢复机制
...
- ComfyUI 客户端（comfyui-image-gen）
  ...
  - **新增**：标准化 API 端点处理，统一的 /prompt 和 /history 接口。
  - **新增**：改进的输出目录解析机制，支持 CLI 参数 > 环境变量 > 默认目录的优先级。
- 画布设计（canvas-design）
...
- 功能要点
  ...
  - **新增**：改进的输出目录解析机制，优先级为：CLI --output > 环境变量 COMFY_OUT > 默认目录。
  - 子命令：krea2、edit、ernie、zimage、klein，分别加载对应 workflow JSON 并注入参数。
  - **新增**：所有工作流均支持批量提示词处理，每行文本作为独立图像生成。
  - 结果收集：轮询 /history/{prompt_id}，下载 images 并保存至输出目录，打印 RESULT 数组。
- 关键实现
  - **标准化 API 端点**：统一的 _post() 和 _get() 方法处理 HTTP 请求。
  - **增强的输出目录解析**：resolve_out_dir() 函数支持多级优先级配置。
...
- **新增**：所有工作流现在都支持批量处理，每个子命令都会自动处理多行提示词。

```mermaid
flowchart TD
Start(["进入子命令"]) --> LoadWF["加载对应 workflow JSON"]
LoadWF --> InjectParams["注入参数<br/>Prompt/Seed/Steps/Size/LoRA"]
InjectParams --> CheckBatch{"是否批量?"}
CheckBatch --> |是| SplitLines["拆分多行提示词"]
CheckBatch --> |否| Submit["POST /prompt"]
SplitLines --> ForEachLine["逐行处理"]
ForEachLine --> Submit
Submit --> Poll{"是否完成?"}
Poll --> |否| Wait["等待并重试"]
...
## 性能与并发优化
- 批量处理
  - **新增**：所有工作流现在都支持批量提示词处理，每行文本作为独立图像生成。
  - 每张图独立种子（可选），保证多样性。
- 内存与 VRAM
...
- 常见问题
  ...
  - **新增**：输出目录问题：检查 --output 参数或 COMFY_OUT 环境变量设置。
- 调试步骤
...
**最新更新**：增强了标准化的 API 端点处理、改进了输出目录解析机制，并在所有工作流中添加了批量处理支持，进一步提升了系统的稳定性和用户体验。
