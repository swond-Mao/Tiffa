...
## 更新摘要
**所做更改**   
- 更新了默认输出目录配置，从 `$PORTABLE_ROOT/workspace/comfyui_out` 改为当前工作目录下的 `comfyui_out`
- 修正了命令行参数帮助文本中的输出目录说明
- 增强了输出目录解析逻辑，支持更灵活的路径管理
- 优化了环境变量处理机制
...
**更新** 默认输出目录已更改为当前工作目录下的 `comfyui_out`，提供更直观的文件管理体验。
...
**图表来源** 
- [skills/comfyui-image-gen/comfy.py:1-452](file://skills/comfyui-image-gen/comfy.py#L1-L452)
...
## 核心组件
- CLI 主程序 comfy.py
  - 环境变量与默认值：COMFY_URL（远程服务器）、COMFY_OUT（输出目录）、PORTABLE_ROOT（便携包根目录）
  - HTTP 封装：_post/_get 用于提交 prompt、轮询历史、下载图片
  - Multipart 表单处理：upload_image 函数支持图片上传到 ComfyUI 服务器
...
**更新** 默认输出目录逻辑已更新，现在优先使用当前工作目录作为基础路径。
...
**图表来源** 
- [skills/comfyui-image-gen/comfy.py:76-121](file://skills/comfyui-image-gen/comfy.py#L76-L121)
- [skills/comfyui-image-gen/workflow_api.json:1-29](file://skills/comfyui-image-gen/workflow_api.json#L1-L29)
- [skills/comfyui-image-gen/workflow_edit_api.json:1-24](file://skills/comfyui-image-gen/workflow_edit_api.json#L1-L24)
- [skills/comfyui-image-gen/workflow_ernie_turbo_api.json:1-26](file://skills/comfyui-image-gen/workflow_ernie_turbo_api.json#L1-L26)
- [skills/comfyui-image-gen/workflow_klein_api.json:1-169](file://skills/comfyui-image-gen/workflow_klein_api.json#L1-L169)
...
**图表来源** 
- [skills/comfyui-image-gen/comfy.py:180-241](file://skills/comfyui-image-gen/comfy.py#L180-L241)
- [skills/comfyui-image-gen/workflow_krea2_api.json:1-158](file://skills/comfyui-image-gen/workflow_krea2_api.json#L1-L158)
...
**图表来源** 
- [skills/comfyui-image-gen/workflow_ernie_turbo_api.json:1-26](file://skills/comfyui-image-gen/workflow_ernie_turbo_api.json#L1-L26)
- [skills/comfyui-image-gen/comfy.py:263-297](file://skills/comfyui-image-gen/comfy.py#L263-L297)
...
**图表来源** 
- [skills/comfyui-image-gen/workflow_zimage_api.json:1-155](file://skills/comfyui-image-gen/workflow_zimage_api.json#L1-L155)
- [skills/comfyui-image-gen/comfy.py:345-379](file://skills/comfyui-image-gen/comfy.py#L345-L379)
...
**图表来源** 
- [skills/comfyui-image-gen/workflow_klein_api.json:1-169](file://skills/comfyui-image-gen/workflow_klein_api.json#L1-L169)
- [skills/comfyui-image-gen/comfy.py:299-343](file://skills/comfyui-image-gen/comfy.py#L299-L343)
...
- 适用场景：P图、换背景、换衣、修饰
- 技术要点：
  - 上传本地图片到 ComfyUI 服务器（使用 multipart/form-data）
...
User->>CLI : 提供图片和编辑指令
CLI->>Upload : 上传本地图片 (multipart/form-data)
Upload-->>CLI : 返回远程文件名
CLI->>EditWF : 加载编辑工作流
...
**图表来源** 
- [skills/comfyui-image-gen/workflow_edit_api.json:1-24](file://skills/comfyui-image-gen/workflow_edit_api.json#L1-L24)
- [skills/comfyui-image-gen/comfy.py:243-261](file://skills/comfyui-image-gen/comfy.py#L243-L261)
...
- CLI 依赖
  ...
  - 环境变量：COMFY_URL、COMFY_OUT、PORTABLE_ROOT
- 网络依赖
...
**更新** 默认输出目录逻辑已更新，不再依赖 PORTABLE_ROOT/workspace 路径。
...
CLI --> ENV["环境变量<br/>COMFY_URL, COMFY_OUT, PORTABLE_ROOT"]
...
- 内存管理：使用流式处理减少内存占用，特别是在处理大图片和批量任务时

**更新** 增强了错误处理和超时管理机制。

**章节来源**
- [skills/comfyui-image-gen/comfy.py:100-121](file://skills/comfyui-image-gen/comfy.py#L100-L121)
- [skills/comfyui-image-gen/comfy.py:76-121](file://skills/comfyui-image-gen/comfy.py#L76-L121)
...
- 图片上传失败
  ...
  - 验证 multipart/form-data 格式正确性
- 输出目录权限
  - 确保 COMFY_OUT 指定的目录有写入权限
  - 默认目录为当前工作目录下的 `comfyui_out`，可通过 --output 参数覆盖
- 环境变量配置
  - 检查 PORTABLE_ROOT 是否正确设置
  - 验证 COMFY_OUT 路径的有效性

**更新** 新增了默认输出目录变更和环境变量配置相关的故障排查指导。

**章节来源**
- [skills/image-gen-router/SKILL.md:96-98](file://skills/image-gen-router/SKILL.md#L96-L98)
- [skills/comfyui-image-gen/comfy.py:32-46](file://skills/comfyui-image-gen/comfy.py#L32-L46)
- [skills/comfyui-image-gen/comfy.py:53-73](file://skills/comfyui-image-gen/comfy.py#L53-L73)

## 结论
ComfyUI 图像生成技能提供了完整的五套工作流管道，支持文生图、图编辑等多种场景。通过统一的 CLI 接口和智能路由机制，用户可以根据需求自动选择最佳管道。系统具备良好的扩展性和容错性，适合大规模批量处理和自动化集成。

**更新** 经过重大重构后，系统现在具有更健壮的 HTTP 处理、更好的错误处理和更灵活的配置选项，默认输出目录更加直观易用。

## 附录：CLI 使用与示例

### 环境变量配置
- COMFY_URL：远程 ComfyUI 服务器地址（默认 http://47.108.197.247:8188）
- COMFY_OUT：输出目录（默认为当前工作目录下的 `comfyui_out`）
- PORTABLE_ROOT：便携包根目录（用于确定默认输出位置）

### 基本用法
```powershell
$env:PYTHONIOENCODING="utf-8"
& "$env:PORTABLE_ROOT/python/python.exe" "$env:PORTABLE_ROOT/skills/comfyui-image-gen/comfy.py" <子命令> --prompt "提示词" [选项]
```
...
# 从文件读取提示词
comfy.py krea2 --prompt-file prompts.txt
```
...
- --output：输出目录（覆盖 COMFY_OUT 环境变量，默认为当前工作目录下的 comfyui_out）
- --negative：负向提示词（仅 klein 支持）
- --cfg：CFG 缩放因子（仅 klein 支持）
- --sampler：采样器名称（仅 klein 支持）

### 输出管理
- 输出格式：PNG 图片
- 文件命名：JOB_nodeId_filename.png
- 目录结构：按 JOB 分类存储，便于批量处理和管理
- 路径解析：优先使用 --output 参数，其次 COMFY_OUT 环境变量，最后默认目录（当前工作目录下的 comfyui_out）

**更新** 新增了默认输出目录变更和相关的环境变量支持。

**章节来源**
- [skills/image-gen-router/SKILL.md:64-98](file://skills/image-gen-router/SKILL.md#L64-L98)
- [skills/comfyui-image-gen/comfy.py:381-452](file://skills/comfyui-image-gen/comfy.py#L381-L452)
