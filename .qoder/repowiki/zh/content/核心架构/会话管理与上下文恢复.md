...
## 更新摘要
**所做更改**   
- 实现了全面的项目会话树系统，支持跨项目标签全球化管理
- 新增懒加载机制，优化大型项目会话树的渲染性能
- 实现激活和静默标记功能，提供清晰的会话状态指示
- 默认展开包含活动标签的项目，提升用户体验
- 会话列表现在递归收集所有会话，包括分支会话
- 增强了实例清理机制，确保删除/归档操作时正确关闭相关实例
- 实现外科手术式文件操作，处理编码碰撞场景
- 添加Windows文件操作的健壮重试机制
...
- **新增**：全面的项目会话树系统，支持跨项目标签全球化
- **新增**：懒加载机制优化大型项目会话树渲染性能
- **新增**：激活和静默标记功能，提供清晰的状态指示
- **新增**：默认展开包含活动标签的项目
- **新增**：递归会话收集，支持分支会话
...
Main --> SessionTree["会话树系统<br/>懒加载/递归收集"]
UI --> TabGlobal["标签全球化<br/>跨项目管理"]
```
...
- **新增**：项目会话树系统：支持跨项目标签全球化、懒加载、激活/静默标记、默认展开活动项目
...
- [electron/renderer/app.js:240-282](file://electron/renderer/app.js#L240-L282)
- [electron/renderer/app.js:1407-1510](file://electron/renderer/app.js#L1407-L1510)
- [data/agent/config.yml:1-30](file://data/agent/config.yml#L1-L30)
- [AGENTS.md:61-77](file://AGENTS.md#L61-L77)

## 架构总览
下图展示从用户输入到上下文恢复的关键流程，包括新增的项目会话树系统和标签全球化：
...
Note over ST,R : 跨项目标签全球化，默认展开活动项目
```
...
### 全面的项目会话树系统
**新增功能**：实现跨项目标签全球化的会话树管理系统

- **懒加载机制**：项目展开时才加载会话列表，避免初始加载大量数据
- **递归会话收集**：支持分支会话子目录 `*_<uuid>/` 内的 .jsonl 文件
- **激活/静默标记**：当前活动会话高亮显示，打开的会话标记为 open 状态
- **默认展开**：包含活动标签的项目默认展开，提升用户体验
- **跨项目标签管理**：`activeTabMeta` 存储全局标签元数据，支持跨项目切换

```mermaid
flowchart TD
ProjectExpand["项目展开"] --> LazyLoad["懒加载会话"]
LazyLoad --> RecursiveScan["递归扫描会话"]
RecursiveScan --> BranchSessions["收集分支会话"]
BranchSessions --> RenderTree["渲染会话树"]
RenderTree --> MarkActive["标记活动会话"]
MarkActive --> DefaultExpand["默认展开活动项目"]
DefaultExpand --> GlobalTabs["跨项目标签管理"]
```

**图表来源**
- [electron/renderer/app.js:240-282](file://electron/renderer/app.js#L240-L282)
- [electron/renderer/app.js:1407-1510](file://electron/renderer/app.js#L1407-L1510)
- [electron/renderer/app.js:2189-2213](file://electron/renderer/app.js#L2189-L2213)
...
- **新增**：会话树优化：懒加载避免初始加载大量数据，递归收集优化分支会话
...
- **新增**：会话树加载缓慢：检查懒加载机制是否正常，确认递归扫描性能
- **新增**：标签全球化异常：检查 `activeTabMeta` 状态管理，确认跨项目切换逻辑
...
## 结论
Tiffa 的会话管理与上下文恢复通过"多实例管理 + JSONL 持久化 + 崩溃自动重启 + Mnemopi 语义记忆"形成闭环，确保用户在跨会话、跨项目、甚至进程崩溃场景下仍能获得连贯的智能体体验。**新增的全面项目会话树系统**提供了跨项目标签全球化、懒加载、激活/静默标记等功能，**增强的实例清理机制**确保了删除和归档操作的可靠性，**智能滚动跟随机制**提升了用户的交互体验，**外科手术式文件操作**解决了编码碰撞场景的问题，**健壮的重试机制**保证了Windows环境下的稳定性。其设计兼顾性能与可靠性，适合弱模型与本地优先的使用场景。
...
- **新增**：会话树管理：renderSessionTreeInto() 实现懒加载和递归会话收集
- **新增**：标签全球化：activeTabMeta 存储跨项目标签元数据，支持持久化恢复
